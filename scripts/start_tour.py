#!/usr/bin/env python
"""SparkWeave Setup Tour — minimal terminal-first guided installer."""
...
def _tour_banner() -> None:
    banner(
        "SparkWeave Setup Tour",
        [
            "Configure your local SparkWeave environment.",
...
def _run_web_tour() -> None:
    ...
    profile = select(
        ...
        [
            ("web-basic", "web-basic", "FastAPI + Vite"),
    ...
    frontend_env["FRONTEND_PORT"] = str(ports["frontend"])

    backend_cmd = [_PYTHON, "-m", "sparkweave.api.run_server"]
    ...
    log_info("Restarting SparkWeave with your configuration ...")
    print()
    launch_at = None
    if cache:
        try:
...
def _run_cli_tour() -> None:
    ...
    print()
    print(f"  {dim('$')} sparkweave chat")
    print(f"  {dim('$')} sparkweave kb list")
    print(f"  {dim('$')} sparkweave serve --port {ports['backend']}")
    print()
...
def run_tour() -> None:
    ...
    step(1, "?", "Choose mode")
    mode = select(
        "How would you like to use SparkWeave?",
        [
