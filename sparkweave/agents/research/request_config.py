...
def build_research_runtime_config(
    *,
    ...
    research_root = {}
    if isinstance(base_config.get("research"), dict):
        research_root.update(base_config["research"])
    if isinstance(capabilities.get("research"), dict):
        research_root.update(capabilities["research"])
    researching_root = (
...
def _build_mode_policy(mode: ResearchMode, depth: ResearchDepth) -> dict[str, Any]:
    from sparkweave.agents.research.mode_strategy import get_strategy
