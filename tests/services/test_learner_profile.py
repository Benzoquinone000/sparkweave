...
async def test_learner_profile_aggregates_existing_evidence(tmp_path) -> None:
    ...
    assert profile["next_action"]["priority"] > 0
    assert profile["quantification"]["version"] == "profile_scoring_v2"
    assert profile["quantification"]["axes"]["weakness"]["score"] > 0
    assert profile["quantification"]["axes"]["evidence"]["score"] > 0
    assert profile["quantification"]["next_action_priority"] == profile["next_action"]["priority"]
    assert profile["data_quality"]["scoring_version"] == "profile_scoring_v2"
    assert profile["data_quality"]["evidence_weight"] > 0
...
async def test_learner_profile_reads_formal_evidence_ledger(tmp_path) -> None:
    ...
    weak_point = next(item for item in profile["learning_state"]["weak_points"] if item["label"] == "学习率判断错误")
    mastery = next(item for item in profile["learning_state"]["mastery"] if item["concept_id"] == "learning-rate")
    assert weak_point["score"] > 0
    assert weak_point["evidence_weight"] > 0
    assert mastery["confidence"] > 0
    assert mastery["evidence_weight"] > 0
