...
def test_mimic_upload_rejects_invalid_base64_pdf(tmp_path: Path) -> None:
    question_router_module = _load_question_router_module()
    question_router_module.MIMIC_OUTPUT_DIR = tmp_path / "mimic_papers"
    ...
    assert "Invalid base64 PDF data" in error["content"]
    assert not list((tmp_path / "mimic_papers").rglob("exam.pdf"))
