...
class TestSparkBotCronRoutes:
    def _client(self, monkeypatch):
        from sparkweave.services.sparkbot import BotConfig

        calls: dict[str, Any] = {"jobs": []}
        ...
        class FakeMgr:
            ...
            def list_cron_jobs(self, bot_id: str, *, include_disabled: bool = True):
            ...
            def add_cron_job(self, bot_id: str, **kwargs):
                ...
                job = {
                    ...
                    "enabled": True,
                    "schedule": {
                        "kind": kwargs["schedule"].kind,
                        "everyMs": kwargs["schedule"].every_ms,
                        "expr": kwargs["schedule"].expr,
                        "tz": kwargs["schedule"].tz,
                        "atMs": kwargs["schedule"].at_ms,
                    },
                    "payload": {"message": kwargs["message"], "channel": kwargs["channel"], "to": kwargs["to"]},
    ...
    def test_create_every_cron_job(self, monkeypatch):
        client, calls = self._client(monkeypatch)

        resp = client.post(
            "/api/v1/sparkbot/b/cron",
            json={
                "name": "Daily check",
                "message": "Check MCP and skills",
                "kind": "every",
                "every_seconds": 60,
                "channel": "feishu",
                "to": "group-a",
        ...
        assert resp.status_code == 200
        assert resp.json()["schedule"]["kind"] == "every"
        assert calls["add"]["schedule"].every_ms == 60_000
        assert calls["add"]["message"] == "Check MCP and skills"
        assert calls["add"]["channel"] == "feishu"
        assert calls["add"]["to"] == "group-a"

    def test_cron_list_toggle_delete_and_run_conflict(self, monkeypatch):
        client, calls = self._client(monkeypatch)

        list_resp = client.get("/api/v1/sparkbot/b/cron?include_disabled=false")
        assert list_resp.status_code == 200
        assert calls["list_include_disabled"] is False

        toggle_resp = client.patch("/api/v1/sparkbot/b/cron/job-1", json={"enabled": False})
        assert toggle_resp.status_code == 200
        assert calls["toggle"] == ("job-1", False)

        delete_resp = client.delete("/api/v1/sparkbot/b/cron/job-1")
        assert delete_resp.status_code == 200
        assert calls["delete"] == "job-1"

        run_resp = client.post("/api/v1/sparkbot/b/cron/job-1/run")
        assert run_resp.status_code == 409
        assert "must be running" in run_resp.json()["detail"].lower()


class TestSparkBotSkillRoutes:
    def _client(self, monkeypatch):
        from sparkweave.services.sparkbot import BotConfig
        ...
        class FakeMgr:
            ...
            def list_bot_skills(self, bot_id: str):
                calls["list"] = bot_id
                return [
                    {
                        "name": "daily-review",
            ...
            def read_bot_skill(self, bot_id: str, skill_name: str):
                calls["read"] = skill_name
            ...
            def write_bot_skill(self, bot_id: str, skill_name: str, content: str):
                calls["write"] = (skill_name, content)
                if not content:
                    raise ValueError("Skill content is required")
                return {"name": skill_name, "content": content, "source": "workspace"}

            def upload_bot_skill(self, bot_id: str, *, filename: str, content: bytes, skill_name: str | None = None):
                calls["upload"] = (filename, content, skill_name)
                return {
                    "name": skill_name or "uploaded-skill",
    ...
    def test_list_read_and_write_skills(self, monkeypatch):
        client, calls = self._client(monkeypatch)

        list_resp = client.get("/api/v1/sparkbot/b/skills")
        assert list_resp.status_code == 200
        assert list_resp.json()["skills"][0]["name"] == "daily-review"

        read_resp = client.get("/api/v1/sparkbot/b/skills/daily-review")
        assert read_resp.status_code == 200
        assert read_resp.json()["content"] == "# Skill"
        assert calls["read"] == "daily-review"

        write_resp = client.put(
            "/api/v1/sparkbot/b/skills/new-skill",
            json={"content": "# New skill"},
        )
        assert write_resp.status_code == 200
        assert calls["write"] == ("new-skill", "# New skill")

    def test_upload_skill_file(self, monkeypatch):
        client, calls = self._client(monkeypatch)

        resp = client.post(
            "/api/v1/sparkbot/b/skills/upload",
            data={"skill_name": "uploaded"},
            files={"file": ("SKILL.md", b"# Uploaded skill", "text/markdown")},
        )

        assert resp.status_code == 200
        assert resp.json()["name"] == "uploaded"
        assert calls["upload"] == ("SKILL.md", b"# Uploaded skill", "uploaded")
