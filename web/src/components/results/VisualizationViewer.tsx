...
function SvgPreview({ svg }: { svg: string }) {
  const sanitized = useMemo(() => sanitizeSvgMarkup(svg), [svg]);
  ...
  useEffect(() => {
    ...
    async function renderDiagram() {
      ...
      try {
        ...
        if (!cancelled) {
          const sanitized = sanitizeSvgMarkup(rendered.svg);
          if (!sanitized) throw new Error("Mermaid rendered invalid SVG.");
...
export function VisualizationViewer({ result }: { result: VisualizeResult }) {
  ...
  return (
    <div className="dt-dynamic-result rounded-lg border border-line bg-canvas p-2.5">
      ...
      {result.response ? <p className="mt-2.5 text-xs leading-5 text-slate-600">{result.response}</p> : null}
