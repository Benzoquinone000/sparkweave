...
class KnowledgeBaseInitializer:
    ...
    def _initial_retrieval_mode(self) -> str | None:
        try:
            from sparkweave.services.kb_config import KnowledgeBaseConfigService

            service = KnowledgeBaseConfigService(config_path=self.base_dir / "kb_config.json")
            raw = service.get_search_mode(self.kb_name)
        except Exception:
            return None

        normalized = str(raw or "").strip().lower().replace("-", "_")
        if normalized in {"hybrid", "dense_sparse", "sparse_dense", "sparse", "bm25"}:
            return "hybrid"
        if normalized in {"dense", "vector", "semantic", "naive"}:
            return "dense"
        return None
    ...
    async def process_documents(
        ...
        try:
            retrieval_mode = self._initial_retrieval_mode()
            init_kwargs = {"progress_callback": _on_progress}
            if retrieval_mode:
                init_kwargs["retrieval_mode"] = retrieval_mode
