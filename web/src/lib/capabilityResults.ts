...
export function extractExternalImageResult(resultMetadata: Record<string, unknown> | undefined): ExternalImageResult | null {
  if (!resultMetadata) return null;
  if (resultMetadata.render_type !== "external_image" && !Array.isArray(resultMetadata.images)) return null;
  const images = Array.isArray(resultMetadata.images)
    ? resultMetadata.images.filter((item): item is NonNullable<ExternalImageResult["images"]>[number] => {
        if (!item || typeof item !== "object") return false;
        const record = item as Record<string, unknown>;
        return (
    ...
    : [];
  if (!images.length && !resultMetadata.response) return null;
  return {
    success: resultMetadata.success !== false,
    ...
    fallback_search: resultMetadata.fallback_search === true,
    agent_chain: Array.isArray(resultMetadata.agent_chain)
      ? (resultMetadata.agent_chain as ExternalImageResult["agent_chain"])
      : undefined,
    tool_chain: Array.isArray(resultMetadata.tool_chain)
      ? (resultMetadata.tool_chain as ExternalImageResult["tool_chain"])
      : undefined,
    view_plan: Array.isArray(resultMetadata.view_plan) ? resultMetadata.view_plan.map(String) : undefined,
    reflection_prompt: typeof resultMetadata.reflection_prompt === "string" ? resultMetadata.reflection_prompt : undefined,
  };
}
