...
from sparkweave.logging import ConsoleFormatter
...
def _discover_plugins() -> list[Any]:
    try:
        discover_plugins = import_module("sparkweave.plugins.loader").discover_plugins
    except Exception:
...
def _collect_project_loggers() -> list[logging.Logger]:
    ...
    for parent_name in ("sparkweave", "src"):
    ...
    for name, logger_obj in logging.root.manager.loggerDict.items():
        if not (name.startswith("sparkweave") or name.startswith("src")):
            continue
...
async def _execute_stream(tool_name: str, params: dict[str, Any]) -> AsyncGenerator[str, None]:
    """Run a tool while capturing project logs and yielding SSE events."""
    registry = get_tool_registry()
    tool = registry.get(tool_name)
    if not tool:
...
async def _execute_capability_stream(
    ...
    from sparkweave.core import Attachment, UnifiedContext
    from sparkweave.runtime.runner import LangGraphRunner

    orch = LangGraphRunner()
