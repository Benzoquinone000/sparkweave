...
import { CheckCircle2, Clock3, PanelLeftOpen, PanelRightOpen, X } from "lucide-react";
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="hidden rounded-lg border border-line bg-canvas px-2.5 py-1.5 text-xs font-medium text-slate-600 sm:inline-flex">
            {activeCapabilityLabel} · {formatRuntimeStatus(runtimeStatus)}
          </span>
          <button
            type="button"
      ...
      {collapsed ? (
        <div className="flex h-full flex-col items-center gap-3">
          ...
          <div className="flex min-h-0 flex-1 flex-col items-center gap-2 rounded-lg border border-line bg-white px-2 py-3 text-slate-500">
            ...
            <span className="rounded-md bg-canvas px-1.5 py-1 text-xs">{sessions.length}</span>
          </div>
        </div>
      ) : (
        <SessionHistoryPanel
