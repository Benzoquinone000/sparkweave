...
notebook_router = importlib.import_module(
    "sparkweave.api.routers.question_notebook"
).router
sessions_router = importlib.import_module("sparkweave.api.routers.sessions").router
...
def store(tmp_path: Path, monkeypatch) -> SQLiteSessionStore:
    ...
    monkeypatch.setattr(
        "sparkweave.api.routers.question_notebook.get_sqlite_session_store",
        lambda: instance,
    )
    monkeypatch.setattr(
...
def test_category_create_rejects_blank_name(store: SQLiteSessionStore) -> None:
    with TestClient(_build_app(store)) as client:
        resp = client.post("/api/v1/question-notebook/categories", json={"name": "   "})
    ...
    with TestClient(_build_app(store)) as client:
        resp = client.post(
            ...
            json={
                ...
                "question_id": "q-large",
                "question": "x" * (request_limits.MAX_QUIZ_QUESTION_CHARS + 1),
                "is_correct": False,
            },
        )
...
def test_upsert_entry_rejects_blank_question_id(store: SQLiteSessionStore) -> None:
    session = asyncio.run(store.create_session())
...
def test_upsert_entry_rejects_oversized_option_value(store: SQLiteSessionStore) -> None:
    session = asyncio.run(store.create_session())
    ...
    with TestClient(_build_app(store)) as client:
        resp = client.post(
            ...
            json={
                "session_id": session["id"],
                "question_id": "q-option",
                "question": "Pick one",
                "options": {"A": "x" * (request_limits.MAX_QUIZ_OPTION_VALUE_CHARS + 1)},
                "is_correct": False,
            },
        )
...
def test_quiz_results_rejects_too_many_answers(store: SQLiteSessionStore) -> None:
    session = asyncio.run(store.create_session())
    answer = {
        "question_id": "q1",
        "question": "2+2?",
    ...
    with TestClient(_build_app(store)) as client:
        resp = client.post(
            ...
            json={"answers": [answer] * (request_limits.MAX_QUIZ_RESULT_ITEMS + 1)},
        )
