...
from sparkweave.services.rag_support.file_routing import (
    DocumentType,
    FileTypeRouter,
)
...
class TestClassifyFiles:
    def test_routes_pdf_and_image_to_parser_text_to_text(self, tmp_path: Path) -> None:
        ...
        cls = FileTypeRouter.classify_files([str(pdf), str(txt), str(png), str(docx)])
        assert cls.parser_files == [str(pdf), str(png), str(docx)]
        assert cls.text_files == [str(txt)]
        assert cls.unsupported == []

    def test_legacy_doc_is_not_treated_as_supported_docx(self, tmp_path: Path) -> None:
        doc = tmp_path / "legacy.doc"
        doc.write_bytes(b"\xd0\xcf\x11\xe0")

        cls = FileTypeRouter.classify_files([str(doc)])

        assert FileTypeRouter.get_document_type(str(doc)) == DocumentType.UNKNOWN
        assert cls.unsupported == [str(doc)]
...
class TestQuickHelpers:
    @pytest.mark.parametrize("filename", ["paper.pdf", "diagram.png", "scan.jpeg", "notes.docx"])
