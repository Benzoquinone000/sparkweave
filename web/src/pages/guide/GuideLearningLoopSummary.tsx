  ...
  const rows = [
    {
      ...
      helper: feedback.evidence_quality?.label || "本次学习行为已进入学习记录。",
    },
    {
      label: "概念",
      value: concepts.length ? `${concepts.length} 个` : "待积累",
      helper: concepts[0]?.summary || "系统会继续收集题目、资源和反思记录。",
    },
    {
      label: "路线",
  ...
  return (
    <div className={`mt-4 rounded-lg border border-line bg-canvas p-3 ${compact ? "" : "shadow-sm"}`} data-testid="guide-learning-impact-summary">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Badge tone={feedbackTone(feedback.tone)}>记录已同步</Badge>
      </div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <p className="text-sm font-semibold text-ink">学习回执</p>
            <Badge tone="brand">已同步记录</Badge>
          </div>
          <p className="mt-1 text-xs leading-5 text-slate-500">不用自己查报告：系统已经把这次提交串到学习记录和下一步处方里。</p>
        </div>
        <a
          href="/memory"
