...
def infer_retrieval_profile(query: str) -> tuple[str, str]:
    ...
    if _looks_like_fast_query(text, lowered):
        return "fast", "short_direct_lookup"
...
def _looks_like_fast_query(text: str, lowered: str) -> bool:
    question_terms = [
        "what",
        "why",
        "how",
    ...
    if any(term in lowered or term in text for term in question_terms):
        return False
    tokens = re.findall(r"[A-Za-z0-9_]+|[\u4e00-\u9fff]+", text)
    return 1 <= len(tokens) <= 4 and len(text) <= 40
