...
class TestGetAgentParamsLlmProbe:
    ...
    def test_reads_configured_max_tokens(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setattr(
            "sparkweave.services.config.PROJECT_ROOT", project_root,
        )
...
class TestLlmProbeUsesAgentsYaml:
    ...
    async def test_probe_passes_configured_max_tokens(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sparkweave.services.config_test_runner.resolve_llm_runtime_config",
            lambda catalog: _stub_resolved_llm(),
        )
        monkeypatch.setattr(
            "sparkweave.services.config_test_runner.get_token_limit_kwargs",
            _real_get_token_limit_kwargs,
        )
        monkeypatch.setattr(
            "sparkweave.services.llm.complete", _fake_llm_complete,
        )
        monkeypatch.setattr(
            "sparkweave.services.config_test_runner.clear_llm_config_cache", lambda: None,
        )
...
def _stub_resolved_llm():
    ...
    from sparkweave.services.config import ResolvedLLMConfig
...
def _real_get_token_limit_kwargs(model: str, max_tokens: int) -> dict[str, int]:
    ...
    from sparkweave.services.config import uses_max_completion_tokens
