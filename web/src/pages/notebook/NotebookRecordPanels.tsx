  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="font-semibold text-ink">记录本信息</h3>
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-[240px_minmax(0,1fr)]">
        <FieldShell label="名称">
          <TextInput value={name} onChange={(event) => setName(event.target.value)} maxLength={NOTEBOOK_LIMITS.name} data-testid="notebook-meta-name" />
        </FieldShell>
        <FieldShell label="描述">
          <TextInput value={description} onChange={(event) => setDescription(event.target.value)} maxLength={NOTEBOOK_LIMITS.description} data-testid="notebook-meta-description" />
        </FieldShell>
      </div>
    </form>
  );
...
export function RecordEditor({
  ...
  return (
    <section className="rounded-lg border border-line bg-white p-3">
      ...
      >
        <FieldShell label="标题">
          <TextInput value={title} onChange={(event) => setTitle(event.target.value)} maxLength={NOTEBOOK_LIMITS.title} data-testid="record-editor-title" />
        </FieldShell>
        <FieldShell label="摘要">
          <TextArea value={summary} onChange={(event) => setSummary(event.target.value)} maxLength={NOTEBOOK_LIMITS.summary} data-testid="record-editor-summary" />
        </FieldShell>
        <FieldShell label="用户问题">
          <TextArea value={userQuery} onChange={(event) => setUserQuery(event.target.value)} maxLength={NOTEBOOK_LIMITS.userQuery} data-testid="record-editor-user-query" />
        </FieldShell>
        <FieldShell label="输出">
          <TextArea
            value={output}
