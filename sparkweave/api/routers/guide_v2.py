...
class SaveGuideV2ArtifactRequest(BaseModel):
    notebook_ids: list[NotebookIdField] = Field(default_factory=list, max_length=MAX_NOTEBOOK_IDS)
    title: str = Field(default="", max_length=MAX_NOTEBOOK_RECORD_TITLE_CHARS)
...
class GuideV2QuizResultItem(BaseModel):
    ...
    @classmethod
    def _limit_string_list(cls, value):
...
class SubmitGuideV2QuizResultsRequest(BaseModel):
    answers: list[GuideV2QuizResultItem] = Field(default_factory=list, max_length=MAX_QUIZ_RESULT_ITEMS)
    save_questions: bool = True
