...
import { cleanText, isCapabilityId, knowledgeBasesFromSearchParams, ragConfigFromSearchParams } from "./chatPageUtils";
...
export function useChatAutoPrompt({
  ...
  useEffect(() => {
    ...
    const requestedConfig = ragConfigFromSearchParams(params);
    const timer = window.setTimeout(() => {
      autoPromptKeyRef.current = key;
      sendQuickAction(prompt, nextCapability, requestedConfig, {
