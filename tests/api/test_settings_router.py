...
from sparkweave.services.config import ResolvedEmbeddingConfig, ResolvedLLMConfig
from sparkweave.services.embedding_support import client as embedding_client_module
from sparkweave.services.embedding_support import config as embedding_config_module
...
def _reset_runtime_state():
    config_module.clear_llm_config_cache()
...
async def test_update_catalog_invalidates_runtime_caches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    old_llm_config = config_module.get_llm_config()
    ...
    new_llm_config = config_module.get_llm_config()
    new_embedding_client = embedding_client_module.get_embedding_client()

    assert response["catalog"]["services"]["llm"]["profiles"][0]["api_key"] == settings_router.REDACTED_SECRET
    assert response["catalog"]["services"]["embedding"]["profiles"][0]["api_key"] == settings_router.REDACTED_SECRET
...
async def test_apply_catalog_invalidates_runtime_caches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert response["catalog"]["services"]["llm"]["profiles"][0]["api_key"] == settings_router.REDACTED_SECRET
    assert response["catalog"]["services"]["embedding"]["profiles"][0]["api_key"] == settings_router.REDACTED_SECRET
    assert response["env"]["LLM_API_KEY"] == settings_router.REDACTED_SECRET
    assert response["env"]["EMBEDDING_API_KEY"] == settings_router.REDACTED_SECRET
    ...
    assert new_llm_config.base_url == "https://after-apply-llm.example/v1"
    assert new_embedding_client is not old_embedding_client
...
@pytest.mark.asyncio
async def test_redacted_catalog_preserves_existing_secret_on_save(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    initial_catalog = _build_catalog(
        llm_model="gpt-old",
        llm_base_url="https://old-llm.example/v1",
        llm_api_key="existing-llm-key",
        embedding_model="text-embedding-old",
        embedding_base_url="https://old-embedding.example/v1",
        embedding_api_key="existing-embedding-key",
    )
    submitted_catalog = deepcopy(initial_catalog)
    submitted_catalog["services"]["llm"]["profiles"][0]["api_key"] = settings_router.REDACTED_SECRET
    submitted_catalog["services"]["llm"]["profiles"][0]["models"][0]["model"] = "gpt-new"
    submitted_catalog["services"]["embedding"]["profiles"][0]["api_key"] = (
        settings_router.REDACTED_SECRET
    )
    submitted_catalog["services"]["embedding"]["profiles"][0]["models"][0]["model"] = (
        "text-embedding-new"
    )
    ...
    response = await settings_router.update_catalog(
        settings_router.CatalogPayload(catalog=submitted_catalog)
    )
    saved = service.load()

    assert response["catalog"]["services"]["llm"]["profiles"][0]["api_key"] == settings_router.REDACTED_SECRET
    assert saved["services"]["llm"]["profiles"][0]["api_key"] == "existing-llm-key"
    assert saved["services"]["llm"]["profiles"][0]["models"][0]["model"] == "gpt-new"
    assert saved["services"]["embedding"]["profiles"][0]["api_key"] == "existing-embedding-key"
    assert saved["services"]["embedding"]["profiles"][0]["models"][0]["model"] == "text-embedding-new"
