...
from sparkweave.services.guide_generation import BaseAgent, GuideManager
...
class CreateSessionRequest(BaseModel):
    ...
    user_input: str = Field(default="", max_length=MAX_CHAT_MESSAGE_CHARS)
    notebook_id: str | None = Field(default=None, max_length=MAX_CHAT_SESSION_ID_CHARS)  # Optional, single notebook mode
    records: list[dict] | None = None  # Optional, cross-notebook mode with direct records
    notebook_references: list[dict] | None = None

    @field_validator("notebook_references", "records")
    @classmethod
    def _validate_reference_payload(cls, value: list[dict] | None) -> list[dict] | None:
        if value is None:
            return None
        bounded_json(value, field_name="references", max_chars=MAX_CHAT_REFERENCES_JSON_CHARS)
        return value
...
class ChatRequest(BaseModel):
    ...
    session_id: str = Field(..., min_length=1, max_length=MAX_CHAT_SESSION_ID_CHARS)
...
class FixHtmlRequest(BaseModel):
    ...
    session_id: str = Field(..., min_length=1, max_length=MAX_CHAT_SESSION_ID_CHARS)
    bug_description: str = Field(..., min_length=1, max_length=MAX_CHAT_MESSAGE_CHARS)
