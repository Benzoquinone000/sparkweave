...
from sparkweave.agents.solve import PlannerAgent, SolveToolRuntime
...
@pytest.mark.asyncio
async def test_planner_agent_process_returns_serializable_plan_dict(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent = PlannerAgent(config={}, language="en", enable_pre_retrieve=False)
    ...
    async def _fake_call_llm(**kwargs: Any) -> str:
        ...
        return json.dumps(
            {
                "analysis": "Use the definition first.",
                "steps": [{"id": "S1", "goal": "Define convolution"}],
            }
        )

    monkeypatch.setattr(agent, "call_llm", _fake_call_llm)

    plan = await agent.process("What is convolution?", scratchpad=None)

    assert plan["analysis"] == "Use the definition first."
    assert plan["steps"] == [{"id": "S1", "goal": "Define convolution"}]
    assert plan["metadata"]["retrieval"] == "(retrieval disabled)"
    assert plan["metadata"]["replan"] is False


@pytest.mark.asyncio
async def test_planner_agent_process_uses_pre_retrieval_context(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    rag_tool = _FakeTool("rag", [ToolParameter(name="query", type="string")])
    registry = _FakeCoreRegistry({"rag": rag_tool})
    ...
    async def _fake_call_llm(**kwargs: Any) -> str:
        ...
        if "Generate 3 concise" in user_prompt:
            return json.dumps({"queries": ["linear convolution", "discrete convolution"]})
        if "Below are passages" in user_prompt:
            return "aggregated retrieval context"
        planning_prompts.append(user_prompt)
        return json.dumps(
            {
                "analysis": "Plan from retrieved context.",
                "steps": [{"id": "S1", "goal": "Explain the retrieved definition"}],
            }
        )

    monkeypatch.setattr(agent, "call_llm", _fake_call_llm)

    plan = await agent.process("What is convolution?", scratchpad=None, kb_name="signals")

    assert [call[0] for call in registry.exec_calls] == ["rag", "rag"]
    assert [call[1]["query"] for call in registry.exec_calls] == [
        "linear convolution",
        "discrete convolution",
    ]
    assert plan["metadata"]["retrieval"] == "aggregated retrieval context"
    assert plan["steps"][0]["goal"] == "Explain the retrieved definition"
    assert any("aggregated retrieval context" in prompt for prompt in planning_prompts)
