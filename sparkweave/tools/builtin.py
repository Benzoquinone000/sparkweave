    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            ...
            description=(
                ...
                "agent to synthesize. When calling this tool, choose "
                "`retrieval_profile` from the user's intent: fast for bare terms, "
                "acronyms, or short direct lookups with no explanation request, "
                "concept for normal explanatory questions, exact for quotes/chapters/definitions, "
                "code for APIs/functions/errors, formula for proofs/derivations, "
                "guide for learning plans, and broad for comparison or multi-hop tasks. "
                "Do not guess the underlying index type; the RAG backend handles dense, "
                "hybrid, and fallback behavior."
            ),
            parameters=[
                ...
                ToolParameter(
                    ...
                    type="string",
                    description=(
                        "Intent-level retrieval strategy selected by the agent. "
                        "Use fast for bare terms, acronyms, titles, or short direct "
                        "lookups with no explicit explanation request, such as 'PCA'. "
                        "Use concept for normal explanatory conceptual questions; "
                        "exact for source text, chapters, "
                        "definitions, or citations; code for APIs, identifiers, "
                        "functions, or errors; formula for equations, proofs, "
                        "or derivations; guide for learning paths or weak points; "
                        "broad for comparison, summary, or multi-hop questions. "
                        "Use auto only when uncertain."
                    ),
                    required=False,
                    enum=["auto", "fast", "concept", "exact", "code", "formula", "guide", "broad"],
                    default="auto",
                ),
            ],
        )
...
class ExternalVideoSearchTool(_PromptHintsMixin, BaseTool):
        ...
        return ToolDefinition(
            ...
            parameters=[
                ...
                ToolParameter(
                    name="search_depth",
                    type="integer",
                    description="How many provider results to inspect per query.",
                    required=False,
                    default=8,
                ),
                ToolParameter(
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            search_depth = int(kwargs.get("search_depth") or 8)
        except (TypeError, ValueError):
            search_depth = 8
...
class ExternalImageSearchTool(_PromptHintsMixin, BaseTool):
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="external_image_search",
            description=(
                "Find and rank a concise set of public learning images, diagrams, or visual references "
                "for the current learning task. Use this when the learner asks to find existing images, "
                "illustrations, image assets, or diagram references from the web."
            ),
            parameters=[
                ToolParameter(
                    name="topic",
                    type="string",
                    description="The learning topic or question to search images for.",
                ),
                ToolParameter(
                    name="prompt",
                    type="string",
                ...
                ToolParameter(
                    name="max_results",
                    type="integer",
                    description="Maximum number of image cards to return.",
                    required=False,
                    default=4,
                ),
                ToolParameter(
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from sparkweave.services.image_search import recommend_learning_images
        ...
        if not isinstance(learner_hints, dict):
            learner_hints = {}
        try:
            max_results = int(kwargs.get("max_results") or 4)
        except (TypeError, ValueError):
            max_results = 4
        ...
        if not topic:
            return ToolResult(
                content="请先告诉我要找哪一个学习主题的图片。",
                success=False,
                metadata={
                    "render_type": "external_image",
                    "images": [],
                    "tool_name": "external_image_search",
                },
            )

        result = await recommend_learning_images(
            topic=topic,
            learner_hints=learner_hints,
            prompt=prompt,
            language=language,
        ...
        metadata["render_type"] = "external_image"
        metadata["tool_name"] = "external_image_search"
        metadata["agent_chain"] = []
        metadata["tool_chain"] = [
            {"label": "精选图片工具", "state": "done", "detail": "检索公开图片和示意图候选。"},
            {"label": "排序筛选", "state": "done", "detail": "按学习任务和画像提示筛选。"},
        ]

        response = str(metadata.get("response") or "").strip()
        if not response:
            response = "我为你筛选了几张适合当前任务的学习图片。"
        sources = [
            {
                "type": "external_image",
                "title": str(image.get("title") or "学习图片"),
            ...
            for image in images
            if isinstance(image, dict) and (image.get("url") or image.get("image_url") or image.get("thumbnail"))
        ]
        return ToolResult(
            content=response,
            success=bool(metadata.get("success", True)),
        ...
        def _sink(event_type: str, payload: dict[str, Any]) -> None:
            ...
            if inspect.isawaitable(result):
                try:
                    ...
                    logger.debug("Skipped async image progress event outside a running loop.")
