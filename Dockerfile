# ============================================
# SparkWeave Multi-Stage Dockerfile
# ============================================
# This Dockerfile builds a production-ready image for SparkWeave
# containing both the FastAPI backend and Vite frontend
#
...
# Build with a local fallback; runtime deployments inject the browser API URL
# through /runtime-config.js.
RUN echo "VITE_API_BASE=http://localhost:${BACKEND_PORT}" > .env.local
...
# Labels
LABEL maintainer="SparkWeave Team" \
      description="SparkWeave: multi-agent personalized learning workbench" \
      version="1.0.0"
...
# Copy built Vite frontend and its tiny static server.
COPY --from=frontend-builder /app/web/dist/ ./web/dist/
COPY --from=frontend-builder /app/web/scripts/static-server.mjs ./web/scripts/static-server.mjs
...
RUN cat > /etc/supervisor/conf.d/sparkweave.conf <<'EOF'
[supervisord]
nodaemon=true
logfile=/dev/null
logfile_maxbytes=0
...
RUN sed -i 's/\r$//' /etc/supervisor/conf.d/sparkweave.conf
...
exec python -m uvicorn sparkweave.api.main:app --host 0.0.0.0 --port ${BACKEND_PORT}
EOF

RUN sed -i 's/\r$//' /app/start-backend.sh && chmod +x /app/start-backend.sh

# Create frontend startup script
# This script handles runtime API base injection for the Vite SPA.
...
# Priority: VITE_API_BASE > NEXT_PUBLIC_API_BASE_EXTERNAL > NEXT_PUBLIC_API_BASE > auto-detect
if [ -n "$VITE_API_BASE" ]; then
    API_BASE="$VITE_API_BASE"
    echo "[Frontend] 📌 Using Vite API URL: ${API_BASE}"
...
echo "[Frontend] Starting SparkWeave Vite frontend on port ${FRONTEND_PORT}..."

cat > /app/web/dist/runtime-config.js <<RUNTIME
window.__SPARKWEAVE_RUNTIME_CONFIG__ = { apiBase: "${API_BASE}" };
RUNTIME

export HOSTNAME=0.0.0.0
export FRONTEND_PORT=${FRONTEND_PORT}
exec node /app/web/scripts/static-server.mjs
EOF
...
from sparkweave.services.setup import init_user_directories
...
# Start supervisord
exec /usr/bin/supervisord -c /etc/supervisor/conf.d/sparkweave.conf
EOF
...
[program:backend]
command=python -m uvicorn sparkweave.api.main:app --host 0.0.0.0 --port %(ENV_BACKEND_PORT)s --reload
directory=/app
autostart=true
autorestart=true
stdout_logfile=/dev/fd/1
...
[program:frontend]
command=/bin/bash -c "cd /app/web && VITE_API_BASE=${VITE_API_BASE:-${NEXT_PUBLIC_API_BASE_EXTERNAL:-${NEXT_PUBLIC_API_BASE:-http://localhost:${BACKEND_PORT:-8001}}}} FRONTEND_PORT=${FRONTEND_PORT:-3782} npm run dev"
directory=/app/web
autostart=true
autorestart=true
startsecs=5
