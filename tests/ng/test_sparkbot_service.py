...
def test_sparkbot_manager_writes_reads_and_lists_workspace_skills(manager):
    manager.save_bot_config("demo", BotConfig(name="Demo"))

    written = manager.write_bot_skill(
        "demo",
        "daily-review",
        "---\n"
        "description: Daily review agent\n"
        "always: true\n"
        "---\n"
        "# Daily Review\n\n"
        "Check pending learner tasks and produce a short plan.",
    )

    assert written["name"] == "daily-review"
    assert written["source"] == "workspace"
    assert written["editable"] is True
    assert written["description"] == "Daily review agent"
    assert written["always"] is True
    assert "pending learner tasks" in written["content"]
    skill_file = manager._workspace_dir("demo") / "skills" / "daily-review" / "SKILL.md"
    assert skill_file.read_text(encoding="utf-8") == written["content"]

    listed = {item["name"]: item for item in manager.list_bot_skills("demo")}
    assert listed["daily-review"]["editable"] is True
    assert listed["daily-review"]["description"] == "Daily review agent"
    assert manager.read_bot_skill("demo", "daily-review")["content"] == written["content"]

    with pytest.raises(ValueError, match="required"):
        manager.write_bot_skill("demo", "empty", "   ")


def test_sparkbot_manager_uploads_zipped_skill_package(manager):
    manager.save_bot_config("demo", BotConfig(name="Demo"))
    archive_bytes = BytesIO()
    with zipfile.ZipFile(archive_bytes, "w") as archive:
        archive.writestr(
            "agent-scheduler/SKILL.md",
            "---\n"
            "description: Scheduled agent skill\n"
            "---\n"
            "# Agent Scheduler\n\n"
            "Run scheduled channel checks.",
        )
        archive.writestr("agent-scheduler/scripts/check.py", "print('ok')\n")
        archive.writestr("agent-scheduler/../escape.txt", "nope")
        archive.writestr("ignored/SKILL.md", "# Ignored")

    uploaded = manager.upload_bot_skill(
        "demo",
        filename="agent-scheduler.zip",
        content=archive_bytes.getvalue(),
    )

    assert uploaded["name"] == "agent-scheduler"
    assert uploaded["description"] == "Scheduled agent skill"
    skill_dir = manager._workspace_dir("demo") / "skills" / "agent-scheduler"
    assert "Run scheduled channel checks" in (skill_dir / "SKILL.md").read_text(encoding="utf-8")
    assert (skill_dir / "scripts" / "check.py").read_text(encoding="utf-8") == "print('ok')\n"
    assert not (skill_dir / "escape.txt").exists()
    assert not (manager._workspace_dir("demo") / "skills" / "ignored").exists()
