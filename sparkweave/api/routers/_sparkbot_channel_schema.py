...
Used by ``GET /api/v1/sparkbot/channels/schema`` so the front-end can render
generic forms for ANY channel (built-in or plugin) without hard-coding fields.

Why live here vs inside the SparkBot channel package?
...
def global_channels_schema() -> dict[str, Any]:
    """Schema for the top-level ``ChannelsConfig`` fields."""
    raw = ChannelsConfig.model_json_schema(by_alias=False)
    flat = inline_refs(raw)
    global_names = ("send_progress", "send_tool_hints", "transcription_api_key")
    properties = flat.get("properties", {})
    if isinstance(properties, dict):
        flat["properties"] = {
            name: properties[name] for name in global_names if name in properties
        }
    required = flat.get("required")
    if isinstance(required, list):
        flat["required"] = [name for name in required if name in global_names]
    return {
        "json_schema": flat,
        "secret_fields": collect_secret_fields(flat),
    }
