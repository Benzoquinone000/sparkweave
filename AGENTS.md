...
## Frontend UX North Star

前端优化最高优先级：面向真实学习用户，而不是展示工程能力。默认入口必须让用户一眼知道下一步点哪里，视觉上保持 Notion 风的简约、低噪声、少层级。

- 一级导航优先保留用户任务：学习、资料、记录、设置。
- Agent、RAG、画像、诊断、演示等工程能力默认后台化或收进“更多工具”。
- 每个页面优先呈现一个主动作，例如继续学习、上传资料、问资料、查看记录。
- 空状态必须给出明确下一步，避免把用户丢进配置项、能力列表或技术名词。
- 新增前端功能时，先判断它服务哪条用户路径，再决定是否暴露为入口。
