...
_SAFE_NOTEBOOK_ID_RE = re.compile(r"^[A-Za-z0-9_-]{1,64}$")
...
class NotebookManager:
    ...
    def _get_notebook_file(self, notebook_id: str) -> Path:
        normalized = str(notebook_id or "").strip()
        if not _SAFE_NOTEBOOK_ID_RE.fullmatch(normalized):
...
def create_notebook_manager(base_dir: str | None = None) -> NotebookManager:
    """Create a notebook manager rooted at an explicit or default directory."""
    return NotebookManager(base_dir=base_dir)


def get_notebooks() -> NotebookManager:
    """Return the shared notebook manager used by the current process."""
