...
class DeepQuestionGraph:
    ...
    def _load_mimic_questions(
        ...
        raw_b64 = str(getattr(attachment, "base64", "") or "").strip()
        content_type = str(getattr(attachment, "mime_type", "") or "").strip()
        if raw_b64.startswith("data:"):
            header, separator, payload = raw_b64.partition(",")
            if not separator:
                raise ValueError("Uploaded PDF attachment has an invalid data URI.")
            header_lower = header.lower()
            if ";base64" not in header_lower:
                raise ValueError("Uploaded PDF attachment must be base64 encoded.")
            ...
            if data_uri_mime:
                if (
                    ...
                    raise ValueError("Uploaded PDF attachment MIME type mismatch.")
                content_type = data_uri_mime
            raw_b64 = payload

        try:
            pdf_bytes = base64.b64decode(raw_b64, validate=True)
        except (binascii.Error, ValueError) as exc:
            raise ValueError("Uploaded PDF attachment is not valid base64.") from exc

        filename = getattr(attachment, "filename", "") or "exam.pdf"
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            pdf_path = Path(tmp_dir) / safe_filename
