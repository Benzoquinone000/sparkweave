...
class TestErrors:
    def test_base_error_str_without_details(self) -> None:
        err = SparkWeaveError("boom")
        assert str(err) == "boom"
        assert err.details == {}

    def test_base_error_str_with_details(self) -> None:
        err = SparkWeaveError("boom", details={"key": "val"})
        assert "key" in str(err)

    def test_hierarchy(self) -> None:
        assert issubclass(ConfigurationError, SparkWeaveError)
        assert issubclass(ValidationError, SparkWeaveError)
        assert issubclass(ServiceError, SparkWeaveError)
