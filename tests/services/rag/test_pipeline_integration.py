...
class TestPipelineIntegration:
    ...
    def test_pipeline(request):
        ...
        if os.getenv("SPARKWEAVE_RAG_LIVE", "").strip().lower() not in {
            "1",
            "true",
            "yes",
            "on",
        }:
            pytest.skip("Set SPARKWEAVE_RAG_LIVE=1 to run real RAG pipeline integration tests.")

        pipeline_name = request.config.getoption("--pipeline")
        if pipeline_name.lower() in {"all", "llamaindex"}:
            from sparkweave.services.rag_support.factory import _llamaindex_runtime_error

            runtime_error = _llamaindex_runtime_error()
            if runtime_error:
                pytest.skip(
                    "LlamaIndex runtime is unavailable in this Python environment: "
                    f"{runtime_error.splitlines()[-1]}"
                )
