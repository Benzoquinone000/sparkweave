...
_DENY_COMMAND_PATTERNS = (
    ...
    r"\brmdir\s+/s\b",
    r"\brd\s+/s\b",
    r"\bremove-item\b.*-(?:r|recurse|force)\b",
    r"(?:^|[;&|]\s*)format\b",
    r"\b(mkfs|diskpart)\b",
    r"\bdd\s+if=",
    r">\s*/dev/sd",
    ...
    r"\bpowershell(?:\.exe)?\b.*\b(?:-encodedcommand|invoke-expression|iex)\b",
    r"\b(?:curl|wget)\b[^|;&]*\|\s*(?:sh|bash|powershell|pwsh)\b",
    r":\(\)\s*\{.*\};\s*:",
...
def _validate_url(url: str) -> str | None:
    ...
    if host in {"localhost", "localhost.localdomain", "host.docker.internal"} or host.endswith(".local"):
        return "Local network hosts are not allowed"
    try:
...
class WebFetchTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            final_url_error = _validate_url(str(response.url))
            if final_url_error:
                return ToolResult(
                    content=f"Error: final URL validation failed: {final_url_error}",
                    success=False,
