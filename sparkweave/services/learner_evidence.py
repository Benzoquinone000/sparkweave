...
def _infer_notebook_resource_type(record_type: str, metadata: dict[str, Any]) -> str:
    ...
    external_image = metadata.get("external_image")
    if isinstance(external_image, dict) and (
        external_image.get("render_type") == "external_image" or isinstance(external_image.get("images"), list)
    ):
    ...
    if capability == "external_video_search":
        return "external_video"
    if capability == "external_image_search":
        return "external_image"
...
def _chat_capability_resource_type(capability: str) -> str:
    mapping = {
        "math_animator": "video",
        "external_image_search": "external_image",
