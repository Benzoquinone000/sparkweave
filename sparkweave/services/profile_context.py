...
class ProfileContextInjector:
    ...
    def _extract_hints(self, profile: dict[str, Any]) -> dict[str, Any]:
        ...
        quantification = _dict(profile.get("quantification"))
        axes = _dict(quantification.get("axes"))
        ...
        hints = {
            ...
            "decision_scores": _decision_scores(profile, quantification, axes),
        }
    ...
    def _format_prompt_block(hints: dict[str, Any]) -> str:
        ...
        scores = _dict(hints.get("decision_scores"))
        if scores:
            score_bits = [
                f"profile_confidence={scores.get('profile_confidence')}",
                f"evidence={scores.get('evidence')}",
                f"weakness={scores.get('weakness')}",
                f"mastery={scores.get('mastery')}",
                f"preference={scores.get('preference')}",
                f"next_action_priority={scores.get('next_action_priority')}",
            ]
            lines.append(f"- decision_scores: {'; '.join(bit for bit in score_bits if not bit.endswith('=None'))}")
...
def _preferred_resource_from_preferences(value: Any) -> str:
    ...
    if "external_image" in preferences or "image_resource" in preferences or any("图片" in item or "配图" in item for item in preferences):
...
def _decision_scores(profile: dict[str, Any], quantification: dict[str, Any], axes: dict[str, Any]) -> dict[str, float]:
    if not quantification and not axes:
        return {}
    return {
        "profile_confidence": _round_score(quantification.get("overall_confidence") or profile.get("confidence")),
        "evidence": _round_score(_dict(axes.get("evidence")).get("score")),
        "weakness": _round_score(_dict(axes.get("weakness")).get("score")),
        "mastery": _round_score(_dict(axes.get("mastery")).get("score")),
        "preference": _round_score(_dict(axes.get("preference")).get("score")),
        "next_action_priority": _round_score(quantification.get("next_action_priority")),
    }
...
def _round_score(value: Any) -> float:
    return round(max(0.0, min(_as_float(value), 1.0)), 3)
