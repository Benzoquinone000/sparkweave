"""Runtime mode helpers for CLI/server entry points."""
...
def _resolve_mode() -> RunMode:
    raw = os.environ.get("SPARKWEAVE_MODE", "").strip().lower()
...
def set_mode(mode: RunMode) -> None:
    """Explicitly set the run mode early in entry points."""
    global _current_mode
    _current_mode = mode
    os.environ["SPARKWEAVE_MODE"] = mode.value
...
__all__ = ["RunMode", "get_mode", "is_cli", "is_server", "set_mode"]
