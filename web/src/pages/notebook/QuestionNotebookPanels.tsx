    ...
    <form className="mt-4 border-t border-line pt-4" onSubmit={onSubmit}>
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-3">
        <FieldShell label="答题记录">
          <TextInput value={value.sessionId} onChange={(event) => patch({ sessionId: event.target.value })} maxLength={QUESTION_LIMITS.sessionId} data-testid="question-upsert-session" />
        </FieldShell>
        <FieldShell label="题目编号">
          <TextInput value={value.questionId} onChange={(event) => patch({ questionId: event.target.value })} maxLength={QUESTION_LIMITS.questionId} data-testid="question-upsert-id" />
        </FieldShell>
        <FieldShell label="难度">
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)]">
        <FieldShell label="题干">
          <TextArea value={value.question} onChange={(event) => patch({ question: event.target.value })} maxLength={QUESTION_LIMITS.question} data-testid="question-upsert-question" />
        </FieldShell>
        <div className="grid gap-3">
          <FieldShell label="参考答案">
            <TextInput value={value.correctAnswer} onChange={(event) => patch({ correctAnswer: event.target.value })} maxLength={QUESTION_LIMITS.answer} data-testid="question-upsert-answer" />
          </FieldShell>
          <FieldShell label="解析">
            <TextArea value={value.explanation} onChange={(event) => patch({ explanation: event.target.value })} maxLength={QUESTION_LIMITS.explanation} data-testid="question-upsert-explanation" />
          </FieldShell>
        </div>
      </div>
