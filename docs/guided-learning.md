...
学习处方会附带三步预告：现在做什么、做完提交什么、系统如何回写画像和调整路线。这样用户不需要理解报告里的评估指标，也能知道这条处方会怎样进入下一轮导学。
...
学习报告页面支持直接下载 Markdown。它和“保存到 Notebook”并行：Notebook 用于项目内复盘，Markdown 下载用于比赛提交、答辩材料整理和学习效果评估说明。
...
课程产出包会生成 `demo_preflight`。它把学习画像、导学路线、多智能体资源、练习反馈闭环、学习报告、PPT 骨架、录屏讲稿和提交物清单压缩成一个赛前检查状态。前端只显示“可以录制 / 可排练 / 先补一项”和最小补齐建议，避免赛前准备页重新变复杂。
...
课程产出包会生成 `recording_script`。它把演示路线压缩成 5 段录屏讲稿，每段包含时间、画面、讲述词和兜底动作。前端只在录屏检查卡里显示第一句讲稿提示，完整讲稿写入 Markdown，帮助 7 分钟视频不跑偏。
