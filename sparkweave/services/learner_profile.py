...
class _ProfileBuilder:
    def __init__(self) -> None:
        ...
        self.weighted_evidence_total = 0.0
        self.scored_evidence_weight = 0.0
        self.recent_evidence_score = 0.0
    ...
    def add_weighted_evidence(
        ...
        *,
        ...
        evidence_weight = _evidence_signal_weight(
            source_id=source_id,
        ...
        self.weighted_evidence_total += evidence_weight
        if score is not None:
            self.scored_evidence_weight += evidence_weight
        self.recent_evidence_score = max(self.recent_evidence_score, _recency_score(created_at))
    ...
    def add_weak_point(
        ...
        *,
        ...
        claim_weight = self._claim_weight(source_id, confidence) * math.sqrt(max(1, evidence_count))
        item.evidence_weight += claim_weight
        support_score = _clamp(
            0.16
            + 0.42 * _clamp(confidence, 0.0, 1.0)
            + 0.18 * _clamp(item.evidence_count / 4, 0.0, 1.0)
            + 0.24 * _clamp(item.evidence_weight / 2.4, 0.0, 1.0),
            0.0,
            1.0,
        )
        item.score = max(item.score, round(support_score, 3))
        item.confidence = max(
            item.confidence,
            round(
                _clamp(0.58 * _clamp(confidence, 0.0, 1.0) + 0.42 * item.score, 0.0, 1.0),
                3,
            ),
        )
        ...
        item.severity = (
            "high"
            if item.score >= 0.72 or item.evidence_count >= 3
            else ("medium" if item.score >= 0.42 else "low")
        )
    ...
    def add_mastery(
        ...
        *,
        ...
        claim_weight = self._claim_weight(source_id, 0.78 if score is not None else 0.5) * math.sqrt(
            count
        )
        previous_weight = item.evidence_weight
        item.evidence_count += max(0, evidence_count)
        if score is not None:
            normalized_score = _clamp(float(score), 0.0, 1.0)
            if item.score is None or previous_weight <= 0:
                item.score = normalized_score
            else:
                item.score = round(
                    ((item.score * previous_weight) + (normalized_score * claim_weight))
                    / (previous_weight + claim_weight),
                    3,
                )
        item.evidence_weight += claim_weight
        item.confidence = max(
            ...
            round(
                _clamp(
                    0.22
                    + 0.34 * _clamp(item.evidence_count / 5, 0.0, 1.0)
                    + 0.44 * _clamp(item.evidence_weight / 3, 0.0, 1.0),
                    0.0,
                    0.96,
                ),
                3,
    ...
    def _add_claim(
        ...
        claim_weight = self._claim_weight(source_id, confidence)
        item.evidence_weight += claim_weight
        item.score = round(_combine_signal(item.score, claim_weight), 3)
        item.confidence = max(
            item.confidence,
            round(
                _clamp(0.55 * _clamp(confidence, 0.0, 1.0) + 0.45 * item.score, 0.0, 1.0),
                3,
            ),
        )
        self._touch_source_evidence(source_id, 1)

    def _claim_weight(self, source_id: str, confidence: float) -> float:
        source = self.sources.get(source_id)
        source_confidence = source.confidence if source else _source_reliability(source_id)
        return _clamp(_clamp(confidence, 0.0, 1.0) * source_confidence, 0.04, 1.0)
    ...
    def build(self) -> dict[str, Any]:
        ...
        weak_points = sorted(
            ...
            key=lambda item: (-item.score, -item.confidence, -item.evidence_count, item.label),
        )[:12]
        mastery = sorted(
            [
        ...
        quantification = self._build_quantification(
            ...
            goals=self._filtered_claims(self.goals, "goal"),
            preferences=self._filtered_claims(self.preferences, "preference"),
            strengths=self._filtered_claims(self.strengths, "strength"),
            weak_points=weak_points,
            mastery=mastery,
            next_action=next_action,
        )
        confidence = float(quantification.get("overall_confidence") or 0.05)
        ...
        snapshot = LearnerProfileSnapshot(
            ...
            data_quality={
                ...
                "evidence_count": evidence_count,
                "evidence_weight": quantification.get("evidence_weight", 0.0),
                "scored_evidence_weight": quantification.get("scored_evidence_weight", 0.0),
                "scoring_version": quantification.get("version"),
    ...
    def _build_quantification(
        ...
        *,
        sources: list[dict[str, Any]],
        evidence_count: int,
        goals: dict[str, LearnerProfileClaim],
        preferences: dict[str, LearnerProfileClaim],
        strengths: dict[str, LearnerProfileClaim],
        weak_points: list[LearnerWeakPoint],
        mastery: list[LearnerMastery],
        next_action: dict[str, Any],
    ) -> dict[str, Any]:
        source_weight = sum(
            _source_reliability(str(source.get("source_id") or ""))
            * _clamp(_as_float(source.get("confidence"), 0.0) or 0.0, 0.0, 1.0)
            * math.log1p(max(0, _as_int(source.get("evidence_count"), 0)))
            for source in sources
        )
        evidence_weight = self.weighted_evidence_total + source_weight
        evidence_coverage = _clamp(evidence_weight / 8.0, 0.0, 1.0)
        scored_coverage = _clamp(self.scored_evidence_weight / 4.0, 0.0, 1.0)
        source_diversity = _clamp(len(sources) / 5.0, 0.0, 1.0)
        calibration_strength = _clamp(self.calibration_count * 0.16, 0.0, 0.32)
        assessment_strength = max(
            scored_coverage,
            _clamp((self.assessment_accuracy or 0.0) * 0.75, 0.0, 1.0),
        )
        recency_strength = self.recent_evidence_score or _latest_source_recency(sources)

        goal_score = _claim_axis_score(goals.values())
        preference_score = _claim_axis_score(preferences.values())
        strength_score = _claim_axis_score(strengths.values())
        weakness_score = max((item.score for item in weak_points), default=0.0)
        weakness_confidence = max((item.confidence for item in weak_points), default=0.0)
        mastery_scores = [float(item.score) for item in mastery if item.score is not None]
        mastery_score = round(sum(mastery_scores) / len(mastery_scores), 3) if mastery_scores else 0.0
        mastery_confidence = max((item.confidence for item in mastery), default=0.0)

        overall_confidence = _clamp(
            0.06
            + 0.34 * evidence_coverage
            + 0.17 * source_diversity
            + 0.18 * assessment_strength
            + 0.11 * recency_strength
            + calibration_strength,
            0.05,
            0.94,
        )
        if evidence_count == 0 and not any((goals, preferences, weak_points, mastery)):
            overall_confidence = 0.05

        return {
            "version": "profile_scoring_v2",
            "overall_confidence": round(overall_confidence, 3),
            "evidence_weight": round(evidence_weight, 3),
            "scored_evidence_weight": round(self.scored_evidence_weight, 3),
            "source_diversity": round(source_diversity, 3),
            "recency": round(recency_strength, 3),
            "axes": {
                "goal": _axis("目标明确度", goal_score, _axis_confidence(goals.values()), goals),
                "weakness": _axis("薄弱点定位", weakness_score, weakness_confidence, weak_points),
                "preference": _axis(
                    "偏好稳定度",
                    preference_score,
                    _axis_confidence(preferences.values()),
                    preferences,
                ),
                "evidence": _axis("证据覆盖度", evidence_coverage, overall_confidence, sources),
                "mastery": _axis("掌握稳定度", mastery_score, mastery_confidence, mastery),
                "strength": _axis("优势稳定度", strength_score, _axis_confidence(strengths.values()), strengths),
            },
            "preference_scores": _claim_score_rows(preferences.values(), limit=8),
            "weakness_scores": _weak_point_score_rows(weak_points, limit=8),
            "mastery_scores": _mastery_score_rows(mastery, limit=12),
            "next_action_priority": round(_as_float(next_action.get("priority"), 0.0) or 0.0, 3),
        }
    ...
    def _next_action(
        ...
        *,
        ...
        if weak_points:
            ...
            priority = _clamp(
                0.52 * item.score
                + 0.28 * item.confidence
                + 0.20 * _clamp(item.evidence_count / 4, 0.0, 1.0),
                0.0,
                1.0,
            )
            return {
        ...
        if needs_support is not None:
            mastery_score = needs_support.score if needs_support.score is not None else 0.0
            priority = _clamp(
                0.58 * (1 - mastery_score)
                + 0.24 * needs_support.confidence
                + 0.18 * _clamp(needs_support.evidence_count / 4, 0.0, 1.0),
                0.0,
                1.0,
            )
            return {
        ...
        if goals:
            priority = _clamp(
                0.46 + 0.22 * len(goals) / 3 + 0.20 * (self.assessment_accuracy or 0.0),
                0.0,
                0.82,
            )
            return {
    ...
    def _preferred_resource_hint(self) -> str:
        ...
        if "external_video" in values or "public_video" in values or "公开视频" in values or "公开课" in values:
            return "精选公开视频"
        if "external_image" in values or "image_resource" in values or "图片" in values or "配图" in values:
            return "精选学习图片"
...
def _claims_to_values(bucket: dict[str, LearnerProfileClaim], limit: int) -> list[str]:
    return [
        ...
        for item in sorted(
            bucket.values(),
            key=lambda item: (-item.score, -item.evidence_weight, -item.confidence, -item.evidence_count, item.value),
        )[:limit]
    ]


def _claim_axis_score(items: Any) -> float:
    values = list(items or [])
    if not values:
        return 0.0
    strongest = max((float(getattr(item, "score", 0.0) or 0.0) for item in values), default=0.0)
    coverage = _clamp(
        sum(float(getattr(item, "evidence_weight", 0.0) or 0.0) for item in values) / 3.0,
        0.0,
        1.0,
    )
    density = _clamp(len(values) / 4.0, 0.0, 1.0)
    return round(_clamp(0.58 * strongest + 0.28 * coverage + 0.14 * density, 0.0, 1.0), 3)


def _axis_confidence(items: Any) -> float:
    values = list(items or [])
    if not values:
        return 0.0
    return round(max(float(getattr(item, "confidence", 0.0) or 0.0) for item in values), 3)


def _axis(label: str, score: float, confidence: float, evidence: Any) -> dict[str, Any]:
    return {
        "label": label,
        "score": round(_clamp(float(score or 0.0), 0.0, 1.0), 3),
        "confidence": round(_clamp(float(confidence or 0.0), 0.0, 1.0), 3),
        "evidence_count": _axis_evidence_count(evidence),
    }


def _axis_evidence_count(evidence: Any) -> int:
    if isinstance(evidence, dict):
        return sum(_as_int(getattr(item, "evidence_count", 0), 0) for item in evidence.values())
    if isinstance(evidence, list):
        total = 0
        for item in evidence:
            if isinstance(item, dict):
                total += _as_int(item.get("evidence_count"), 1)
            else:
                total += _as_int(getattr(item, "evidence_count", 1), 1)
        return total
    return 0


def _claim_score_rows(items: Any, *, limit: int) -> list[dict[str, Any]]:
    rows = []
    for item in sorted(
        list(items or []),
        key=lambda value: (
            -float(getattr(value, "score", 0.0) or 0.0),
            -float(getattr(value, "evidence_weight", 0.0) or 0.0),
        ...
        rows.append(
            {
                ...
                "evidence_weight": round(item.evidence_weight, 3),
...
def _weak_point_score_rows(items: list[LearnerWeakPoint], *, limit: int) -> list[dict[str, Any]]:
    return [
        {
            "label": item.label,
            "score": round(item.score, 3),
        ...
        for item in sorted(
            items,
            key=lambda value: (-value.score, -value.confidence, -value.evidence_count, value.label),
        )[:limit]
    ]


def _mastery_score_rows(items: list[LearnerMastery], *, limit: int) -> list[dict[str, Any]]:
    return [
        {
...
def _combine_signal(current: float, incoming: float) -> float:
    return _clamp(
        1 - (1 - _clamp(current, 0.0, 1.0)) * (1 - _clamp(incoming, 0.0, 1.0)),
        0.0,
        1.0,
    )
...
def _source_reliability(source_id: str) -> float:
    ...
    if "profile_calibration" in normalized:
        return 1.0
    if "question" in normalized or "quiz" in normalized:
        return 0.86
...
def _evidence_signal_weight(
    *,
    ...
    verb_weight = {
        "answered": 1.0,
        "completed": 0.92,
        "confirmed_profile": 1.0,
        "corrected_profile": 1.0,
        "stated": 0.72,
        "viewed": 0.58,
        "saved": 0.52,
        "planned": 0.45,
        ...
        "generated": 0.24,
        "observed": 0.38,
    }.get(str(verb or "").strip().lower(), 0.42)
    object_weight = {
        "quiz": 1.0,
        "guide_task": 0.9,
        "learning_blocker": 0.8,
        "learning_goal": 0.72,
        "learning_preference": 0.68,
        "profile_claim": 1.0,
        "resource": 0.52,
        "notebook_record": 0.48,
    }.get(str(object_type or "").strip().lower(), 0.5)
    resource_bonus = 1.05 if resource_type else 1.0
    return round(
        _clamp(weight, 0.0, 2.5)
        * _source_reliability(source_id)
        * _clamp(confidence, 0.0, 1.0)
        * verb_weight
        * object_weight
        * resource_bonus
        * max(0.18, _recency_score(created_at)),
        4,
    )


def _recency_score(value: Any, *, half_life_days: float = 45.0) -> float:
    timestamp = _timestamp_value(value)
    if timestamp is None:
        return 0.48
    age_seconds = max(0.0, time.time() - timestamp)
    half_life_seconds = max(1.0, half_life_days * 24 * 3600)
    return _clamp(0.22 + 0.78 * math.exp(-age_seconds / half_life_seconds), 0.22, 1.0)


def _latest_source_recency(sources: list[dict[str, Any]]) -> float:
    recencies = [_recency_score(source.get("updated_at")) for source in sources if source.get("updated_at")]
    return max(recencies, default=0.0)
...
def _infer_preferences(text: str) -> list[str]:
    lowered = text.lower()
    candidates = [
        ("精选学习图片", ["图片", "配图", "参考图", "image reference", "external_image"]),
        ("图解", ["图解", "可视化", "visual"]),
