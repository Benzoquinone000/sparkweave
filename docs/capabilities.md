...
Guide V2 不是主 WebSocket 的独立 capability，但它会把学习任务映射到 `visualize`、`math_animator`、`deep_question`、`deep_research` 等能力来生成图解、视频、练习和资料；也会通过 `external_video_search` 工具复用联网搜索筛选公开视频。普通对话中，当学习者明确说“找视频 / 推荐视频 / 公开课”时，对话协调器会直接调用精选视频工具，返回视频卡片而不是原始搜索列表。编排链路见 [导学空间与 Guide V2](./guided-learning.md)。
