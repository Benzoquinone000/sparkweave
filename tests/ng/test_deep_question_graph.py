...
@pytest.mark.asyncio
async def test_deep_question_graph_rejects_invalid_uploaded_pdf_base64(monkeypatch):
    parser_called = False
    ...
    context = UnifiedContext(
        ...
        attachments=[
            Attachment(
                ...
                filename="mock_exam.pdf",
                mime_type="application/pdf",
                base64="data:application/pdf;base64,not valid pdf base64",
            )
        ],
        config_overrides={
            "mode": "mimic",
    ...
    assert "not valid base64" in error_events[0].content
    result_events = [event for event in bus._history if event.type == StreamEventType.RESULT]
