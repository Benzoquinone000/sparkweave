...
from sparkweave.services.config_support.provider_catalog import (
...
class EnvStore:
    ...
    def as_summary(self) -> ConfigSummary:
        ...
        if not llm_api_key:
            ...
            if llm_provider == "iflytek_spark_ws":
                ...
                llm_api_key = value("IFLYTEK_MAAS_API_PASSWORD") or value(
        ...
        if (
            not search_api_key
            and search_provider.strip().lower().replace("-", "_") == "iflytek_spark"
        ):
        ...
        return ConfigSummary(
            ...
            embedding={
                ...
                "request_timeout": value("EMBEDDING_REQUEST_TIMEOUT", "60"),
                "batch_size": value("EMBEDDING_BATCH_SIZE", "10"),
                "batch_delay": value("EMBEDDING_BATCH_DELAY", "0"),
            },
            search={
            ...
            ocr={
                ...
                "siliconflow_base_url": value(
                    "SILICONFLOW_OCR_BASE_URL", DEFAULT_SILICONFLOW_OCR_BASE_URL
                ),
                "siliconflow_model": value("SILICONFLOW_OCR_MODEL", DEFAULT_SILICONFLOW_OCR_MODEL),
                "siliconflow_prompt": value(
                    "SILICONFLOW_OCR_PROMPT", DEFAULT_SILICONFLOW_OCR_PROMPT
                ),
            },
            tts={
    ...
    def render_from_catalog(self, catalog: dict[str, Any]) -> dict[str, str]:
        ...
        def is_iflytek_llm() -> bool:
            return (
                canonical_provider_name(_as_str((llm_profile or {}).get("binding")))
                == "iflytek_spark_ws"
            )
        ...
        def is_iflytek_embedding() -> bool:
            return (
                _canonical_embedding_provider_name(
                    _as_str((embedding_profile or {}).get("binding"))
                )
                == "iflytek_spark"
            )
        ...
        def is_siliconflow_endpoint(profile: dict[str, Any] | None) -> bool:
            ...
            return (
                binding == "siliconflow" or provider == "siliconflow" or "siliconflow" in base_url
            )

        def is_iflytek_search() -> bool:
            return (
                _as_str((search_profile or {}).get("provider")).lower().replace("-", "_")
                == "iflytek_spark"
            )

        ocr_provider = str(
            (ocr_profile or {}).get("provider") or current.get("SPARKWEAVE_OCR_PROVIDER", "iflytek")
        )
        ocr_is_siliconflow = ocr_provider.strip().lower().replace("-", "_") in {
            "siliconflow",
            "silicon_flow",
        ...
        return {
            ...
            "LLM_API_KEY": str(
                ...
                else (llm_profile or {}).get("api_key") or ""
            ),
            ...
            "EMBEDDING_API_KEY": str(
                ...
                else (embedding_profile or {}).get("api_key") or ""
            ),
            ...
            "SEARCH_API_KEY": str(
                "" if is_iflytek_search() else (search_profile or {}).get("api_key") or ""
            ),
            ...
            "SPARKWEAVE_OCR_TIMEOUT": str((ocr_profile or {}).get("timeout") or ""),
            "SPARKWEAVE_OCR_MAX_PAGES": str((ocr_profile or {}).get("max_pages") or ""),
            "SPARKWEAVE_OCR_DPI": str((ocr_profile or {}).get("dpi") or ""),
            "SPARKWEAVE_OCR_MIN_TEXT_CHARS": str((ocr_profile or {}).get("min_text_chars") or ""),
            "IFLYTEK_OCR_APPID": str(
                ocr_extra.get("app_id")
                or ("" if iflytek_app_id else current.get("IFLYTEK_OCR_APPID", ""))
            ),
            ...
            "IFLYTEK_TTS_APPID": str(
                tts_extra.get("app_id")
                or ("" if iflytek_app_id else current.get("IFLYTEK_TTS_APPID", ""))
            ),
            ...
            "IFLYTEK_TTS_VOICE": str(
                tts_extra.get("voice")
                or current.get("IFLYTEK_TTS_VOICE", DEFAULT_IFLYTEK_TTS_VOICE)
            ),
            ...
            "IFLYTEK_ASR_APPID": str(
                asr_extra.get("app_id")
                or ("" if iflytek_app_id else current.get("IFLYTEK_ASR_APPID", ""))
            ),
            ...
            "IFLYTEK_SPEECH_EVAL_LANGUAGE": str(
                speech_eval_extra.get("language")
                or current.get("IFLYTEK_SPEECH_EVAL_LANGUAGE", "zh_cn")
            ),
            ...
            "IFLYTEK_FORMULA_ENT": str(
                formula_ocr_extra.get("ent")
                or current.get("IFLYTEK_FORMULA_ENT", DEFAULT_IFLYTEK_FORMULA_ENT)
            ),
            "IFLYTEK_FORMULA_AUE": str(
                formula_ocr_extra.get("aue")
                or current.get("IFLYTEK_FORMULA_AUE", DEFAULT_IFLYTEK_FORMULA_AUE)
            ),
            "IFLYTEK_FORMULA_TIMEOUT": str(
                (formula_ocr_profile or {}).get("timeout")
                or current.get("IFLYTEK_FORMULA_TIMEOUT", "30")
            ),
            ...
            "IFLYTEK_VISION_DOMAIN": str(
                image_understanding_extra.get("domain")
                or current.get("IFLYTEK_VISION_DOMAIN", DEFAULT_IFLYTEK_VISION_DOMAIN)
            ),
            "IFLYTEK_VISION_MAX_TOKENS": str(
                image_understanding_extra.get("max_tokens")
                or current.get("IFLYTEK_VISION_MAX_TOKENS", "2048")
            ),
            "IFLYTEK_VISION_TEMPERATURE": str(
                image_understanding_extra.get("temperature")
                or current.get("IFLYTEK_VISION_TEMPERATURE", "0.2")
            ),
            ...
            "IFLYTEK_VISION_TIMEOUT": str(
                (image_understanding_profile or {}).get("timeout")
                or current.get("IFLYTEK_VISION_TIMEOUT", "45")
            ),
            "IFLYTEK_VISION_UID": str(
                image_understanding_extra.get("uid")
                or current.get("IFLYTEK_VISION_UID", "sparkweave")
            ),
        }
...
class ModelCatalogService:
                ...
                "profiles": [
                    {
                        ...
                        "extra_headers": {
                            ...
                            "service_id": summary.ocr["service_id"]
                            or DEFAULT_IFLYTEK_OCR_SERVICE_ID,
                            "category": summary.ocr["category"] or DEFAULT_IFLYTEK_OCR_CATEGORY,
                            "model": summary.ocr["siliconflow_model"]
                            or DEFAULT_SILICONFLOW_OCR_MODEL,
                            "prompt": summary.ocr["siliconflow_prompt"]
    ...
    def _sync_active_services_from_env(
        ...
        *,
        ...
        def ensure_embedding_profile() -> tuple[dict[str, Any], dict[str, Any]]:
            ...
            model = (
                self.get_active_model(catalog, "embedding") or profile.setdefault("models", [{}])[0]
            )
            return profile, model
        ...
        if {"LLM_BINDING", "LLM_MODEL", "LLM_API_KEY", "LLM_HOST", "LLM_API_VERSION"}.intersection(
            ...
            for env_key, target, value in (
                ...
                field_name = (
                    "base_url" if env_key == "LLM_HOST" else env_key.removeprefix("LLM_").lower()
                )
        ...
        if {"SEARCH_PROVIDER", "SEARCH_API_KEY", "SEARCH_BASE_URL", "SEARCH_PROXY"}.intersection(
            ...
            for env_key, value in (
                ...
                field_name = (
                    "base_url"
                    if env_key == "SEARCH_BASE_URL"
                    else env_key.removeprefix("SEARCH_").lower()
                )
    ...
    def _sync_provider_credentials(self, catalog: dict[str, Any]) -> bool:
        ...
        embedding_binding = _canonical_embedding_provider_name(
            _as_str(embedding_profile.get("binding"))
        )
        ...
        formula_ocr_provider = (
            _as_str(formula_ocr_profile.get("provider")).lower().replace("-", "_")
        )
        ...
        image_understanding_provider = (
            _as_str(image_understanding_profile.get("provider")).lower().replace("-", "_")
        )
        ...
        speech_eval_provider = (
            _as_str(speech_eval_profile.get("provider")).lower().replace("-", "_")
        )
    ...
    def _strip_shared_credentials_from_profiles(self, catalog: dict[str, Any]) -> bool:
        ...
        has_iflytek = any(
            _as_str(iflytek.get(key)) for key in ("app_id", "api_key", "api_secret", "api_password")
        )
        ...
        for profile in services.get("llm", {}).get("profiles", []):
            if (
                canonical_provider_name(_as_str(profile.get("binding"))) == "iflytek_spark_ws"
                and has_iflytek
            ):
        ...
        for profile in services.get("ocr", {}).get("profiles", []):
            ...
            if provider in {"iflytek", "xfyun", "xunfei"} and has_iflytek:
                ...
                provider in {"siliconflow", "silicon_flow", "deepseekocr", "deepseek_ocr"}
                and has_siliconflow
            ):
                clear_profile_key(profile, siliconflow.get("api_key"))
        ...
        for service_name in (
            ...
            for profile in profiles:
                ...
                if service_name == "search":
                    ...
                    ocr_provider = (
                        str(profile.get("provider") or "iflytek").strip().lower().replace("-", "_")
                    )
                    if ocr_provider in {
                        "siliconflow",
                        "silicon_flow",
                    ...
                    if service_name == "llm":
                        ...
                        if llm_spec and llm_spec.name == "iflytek_spark_ws":
                            ...
                            selected_model = ((profile.get("models") or [{}])[0] or {}).get("model")
                    ...
                    for model in models:
                        ...
                        if (
                            service_name == "llm"
                            and llm_spec
                            and llm_spec.name == "iflytek_spark_ws"
                        ):
...
def _inject_runtime_paths(config: dict[str, Any]) -> dict[str, Any]:
    ...
    run_code["workspace"] = _config_path(
        path_service.get_chat_feature_dir("_detached_code_execution")
    )
...
def resolve_config_path(
    ...
    raise FileNotFoundError(
        f"Configuration file not found: {config_file} (expected under {path.parent})"
    )
...
def get_path_from_config(
    config: dict[str, Any], path_key: str, default: str | None = None
) -> str | None:
...
def resolve_llm_runtime_config(
    ...
    *,
    ...
    max_tokens = _as_int((model or {}).get("max_tokens"), 4096)
    temperature = _as_float((model or {}).get("temperature"), 0.7)
...
def resolve_embedding_runtime_config(
    ...
    *,
    ...
    request_timeout = max(1, _safe_int(summary.embedding.get("request_timeout"), 60))
    batch_size = max(1, _safe_int(summary.embedding.get("batch_size"), 10))
    batch_delay = max(0.0, _safe_float(summary.embedding.get("batch_delay"), 0.0))
    ...
    provider_credentials = (
        loaded.get("provider_credentials")
        if isinstance(loaded.get("provider_credentials"), dict)
        else {}
    )
    iflytek_credentials = (
        provider_credentials.get("iflytek")
        if isinstance(provider_credentials.get("iflytek"), dict)
        else {}
    )
    siliconflow_credentials = (
...
def _get_llm_config_from_env() -> LLMConfig:
    ...
    spec = (
        find_by_name(canonical_provider_name(binding) or binding)
        or find_by_name("openai")
        or PROVIDERS[0]
    )
    if spec.name == "iflytek_spark_ws":
        if not api_key:
