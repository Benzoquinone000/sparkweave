...
from sparkweave.services.co_writer import (
    TOOL_CALLS_DIR,
    AgenticChatPipeline,
    EditAgent,
    append_history,
...
_OPERATION_ID_PATTERN = re.compile(r"^[A-Za-z0-9_-]{1,80}$")
MAX_CO_WRITER_TEXT_CHARS = 20_000
MAX_CO_WRITER_INSTRUCTION_CHARS = 2_000
...
def _markdown_download_filename(filename: object) -> str:
    raw_name = str(filename or "document.md")
    try:
        safe_name = DocumentValidator.sanitize_filename(raw_name)
    except ValueError:
...
def _attachment_content_disposition(filename: str) -> str:
    fallback = re.sub(r"[^A-Za-z0-9._-]+", "_", filename).strip("._") or "document.md"
...
class EditRequest(BaseModel):
    text: str = Field(..., min_length=1, max_length=MAX_CO_WRITER_TEXT_CHARS)
    instruction: str = Field(..., min_length=1, max_length=MAX_CO_WRITER_INSTRUCTION_CHARS)
...
class ReactEditRequest(BaseModel):
    selected_text: str = Field(..., min_length=1, max_length=MAX_CO_WRITER_TEXT_CHARS)
    instruction: str = Field(default="", max_length=MAX_CO_WRITER_INSTRUCTION_CHARS)
...
async def export_markdown(content: dict):
    ...
    try:
        ...
        filename = _markdown_download_filename(content.get("filename", "document.md"))
