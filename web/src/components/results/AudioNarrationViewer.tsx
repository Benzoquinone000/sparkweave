...
import { authenticatedResourceUrl } from "@/lib/api";
...
export function AudioNarrationViewer({
  ...
  const playbackUrl = audio?.asset_url ? authenticatedResourceUrl(audio.asset_url) : "";
  const sampleRateLabel = formatSampleRate(audio?.sample_rate);
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        {sampleRateLabel ? <Badge tone="neutral">{sampleRateLabel}</Badge> : null}
      </div>
      ...
      <div className="dt-dynamic-panel mt-4 rounded-lg border border-line bg-white p-3">
        ...
        <audio controls preload="none" className="w-full" src={playbackUrl} />
      </div>
