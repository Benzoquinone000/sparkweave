...
| `sparkweave/services/rag_support/pipelines/llamaindex.py` | 知识库共享文档解析、PDF 文本层解析和 OCR fallback 策略；Milvus pipeline 复用这套解析逻辑 |
...
`sparkweave/services/ocr.py` 当前支持讯飞 OCR 与硅基流动 DeepSeek-OCR。配置对象：
...
| 变量 | 说明 |
| --- | --- |
| `SPARKWEAVE_OCR_PROVIDER` | `iflytek`、`xunfei`、`xfyun`、`siliconflow`、`disabled` |
...
| `SPARKWEAVE_OCR_TIMEOUT` | 请求超时秒数，默认 90 |
| `SPARKWEAVE_OCR_MAX_PAGES` | PDF OCR 最大页数，留空默认不限制 |
| `SPARKWEAVE_OCR_DPI` | PDF 渲染 DPI，默认 200 |
| `SILICONFLOW_OCR_API_KEY` | 硅基流动 API Key |
| `SILICONFLOW_OCR_BASE_URL` | 默认 `https://api.siliconflow.cn/v1` |
| `SILICONFLOW_OCR_MODEL` | 默认 `deepseek-ai/DeepSeek-OCR` |
| `SILICONFLOW_OCR_PROMPT` | DeepSeek-OCR 入库文本提取提示词 |
...
默认 Milvus pipeline 继承并复用这套解析逻辑，差异只在向量写入和检索后端。
...
`SPARKWEAVE_OCR_MIN_TEXT_CHARS` 默认是 40。
...
PDF OCR 会用 PyMuPDF 把每页渲染成 PNG，再逐页调用当前 provider 的 `recognize_image()`：
...
| OCR 配置后没有生效 | `SPARKWEAVE_OCR_PROVIDER` 是否为 `iflytek` 或 `siliconflow`；对应密钥是否完整 |
| 扫描版 PDF 仍为空 | 调低 `SPARKWEAVE_OCR_MIN_TEXT_CHARS`，或使用 `SPARKWEAVE_PDF_OCR_STRATEGY=ocr_first` |
| OCR 请求失败 | 讯飞检查三件套/service_id/category；硅基流动检查 APIKey/base_url/model；同时检查网络和 `SPARKWEAVE_OCR_TIMEOUT` |
