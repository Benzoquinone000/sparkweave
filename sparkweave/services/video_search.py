    ...
    *,
    ...
    for query in queries:
        ...
        for item in _iter_search_items(result):
            ...
            candidate.quality_signals = _quality_signals(candidate, topic=topic, hints=hints, language=language)
    ...
    return {
        ...
        "quality_signals": _aggregate_quality_signals(videos),
...
def _build_queries(*, topic: str, hints: dict[str, Any], prompt: str, language: str) -> list[str]:
    return [item["query"] for item in _build_search_plan(topic=topic, hints=hints, prompt=prompt, language=language)]
...
def _build_search_plan(*, topic: str, hints: dict[str, Any], prompt: str, language: str) -> list[dict[str, str]]:
    ...
    if language.lower().startswith("zh"):
        return [
            {
                "query": f"{focus} 入门 直观 视频 {length_hint}",
                "purpose": "broad beginner-friendly candidate discovery",
                "platform": "web",
            },
            {
                "query": f"{focus} {length_hint} 图解 例题 视频",
                "purpose": "find visual examples that match short learning windows",
                "platform": "web",
            },
            {
                "query": f"{focus} site:bilibili.com/video",
                "purpose": "prefer Chinese public video results when available",
                "platform": "bilibili",
            },
            {
                "query": f"{focus} site:youtube.com/watch",
                "purpose": "include globally available public lectures and tutorials",
                "platform": "youtube",
            },
        ]
    ...
    return [
        {
            "query": f"{focus} beginner intuition {english_length_hint} video",
            "purpose": "broad beginner-friendly candidate discovery",
            "platform": "web",
        },
        {
            "query": f"{focus} visual examples explained {english_length_hint}",
            "purpose": "find visual examples that match short learning windows",
            "platform": "web",
        },
        {
            "query": f"{focus} site:youtube.com/watch",
            "purpose": "include public lectures and tutorials",
            "platform": "youtube",
        },
        {
            "query": f"{focus} site:bilibili.com/video",
            "purpose": "include Chinese public video results when relevant",
            "platform": "bilibili",
        },
    ]
...
def _score_candidate(candidate: VideoCandidate, *, topic: str, hints: dict[str, Any], language: str) -> float:
    ...
    if re.search(r"入门|直观|可视化|动画|图解|例子|基础|讲解|intuition|beginner|visual|explained", haystack):
        score += 0.14
    if re.search(r"例题|案例|demo|example|worked example|step by step|分步", haystack):
        score += 0.06
    if re.search(r"课程|大学|高校|公开课|lecture|course", haystack):
        score += 0.05
    if re.search(r"mit|stanford|harvard|khan academy|3blue1brown|官方|大学|公开课", haystack):
        score += 0.04
    if candidate.platform == "Bilibili" and language.lower().startswith("zh"):
        score += 0.05
    score += _preferred_platform_boost(candidate, hints)
...
def _quality_signals(
    ...
    *,
    ...
    if re.search(r"入门|直观|图解|例子|intuition|beginner|visual|explained", haystack):
        signals.append("beginner_visual")
    if candidate.embed_url:
        signals.append("embeddable")
    if candidate.duration_seconds is not None:
        if 240 <= candidate.duration_seconds <= 1200:
            signals.append("good_duration")
        time_budget = _coerce_minutes(hints.get("time_budget_minutes"))
        if time_budget and candidate.duration_seconds / 60 <= max(5, time_budget * 1.2):
            signals.append("fits_time_budget")
    if candidate.platform == "Bilibili" and language.lower().startswith("zh"):
        signals.append("language_platform_fit")
    if _preferred_platform_boost(candidate, hints) > 0:
        signals.append("matches_platform_preference")
    if re.search(r"课程|公开课|lecture|course|khan academy|3blue1brown|mit|stanford|官方", haystack):
        signals.append("credible_source")
    return signals[:6]


def _aggregate_quality_signals(videos: list[VideoCandidate]) -> list[str]:
    signals: list[str] = []
    for video in videos:
        for signal in video.quality_signals:
            if signal not in signals:
                signals.append(signal)
    return signals


def _preferred_platform_boost(candidate: VideoCandidate, hints: dict[str, Any]) -> float:
    preference_items = [
        *_as_strings(hints.get("preferred_platform")),
        *_as_strings(hints.get("preferred_resource")),
        *_as_strings(hints.get("preferences")),
    ]
    preferences = " ".join(preference_items).lower()
    if not preferences:
        return 0.0
    platform = candidate.platform.lower()
    if platform == "bilibili" and any(term in preferences for term in ("b站", "哔哩哔哩", "bilibili")):
        return 0.06
    if platform == "youtube" and "youtube" in preferences:
        return 0.06
    if any(term in preferences for term in ("公开视频", "公开课", "public video", "lecture")):
        return 0.03
    return 0.0
