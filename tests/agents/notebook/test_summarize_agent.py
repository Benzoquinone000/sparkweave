...
from sparkweave.services.llm import LLMConfig
...
def test_summarize_agent_stores_extra_headers(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "sparkweave.services.notebook_summary.get_llm_config", lambda: cfg,
    )
...
async def test_summarize_agent_forwards_extra_headers(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "sparkweave.services.notebook_summary.llm_stream", _fake_llm_stream,
    )
