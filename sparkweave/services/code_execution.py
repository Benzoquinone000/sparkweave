...
class ImportGuard:
    ...
    @staticmethod
    def _attribute_base_name(node: ast.AST) -> str:
        current = node
    ...
    @staticmethod
    def _import_roots(tree: ast.AST) -> tuple[list[str], dict[str, str]]:
        imported: list[str] = []
            ...
            if isinstance(node, ast.Import):
                ...
                if node.module:
                    ...
                    for alias in node.names:
                        if alias.name == "*":
                            continue
                        local_roots[alias.asname or alias.name] = root

        return imported, local_roots
    ...
    def validate(code: str, allowed_imports: list[str] | None):
        ...
        imported, local_roots = ImportGuard._import_roots(tree)
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Attribute):
                base_name = ImportGuard._attribute_base_name(node)
                base_root = local_roots.get(base_name, base_name)
                if node.attr in DISALLOWED_ATTRIBUTE_NAMES or (
                    node.attr.startswith("__") and node.attr.endswith("__")
                ):
                    raise CodeExecutionError(
                        f"Use of unsafe attribute is not allowed: {node.attr}"
                    )
                if base_root in DISALLOWED_ATTRIBUTE_BASES:
                    raise CodeExecutionError(
                        f"Use of unsafe module access is not allowed: {base_root}.{node.attr}"
                    )
            if isinstance(node, ast.Call):
                ...
                if (
                    ...
                    and local_roots.get(node.func.id) in DISALLOWED_ATTRIBUTE_BASES
                ):
                    raise CodeExecutionError(
                        f"Use of unsafe imported callable is not allowed: {node.func.id}"
                    )
...
async def run_python_code(
    *,
    ...
    """Execute Python code through the NG code runner."""
