"""Unit tests for SparkBotManager config persistence & merging."""
...
@pytest.fixture
def manager(tmp_path: Path) -> SparkBotManager:
    """Return a SparkBotManager whose data dir is a fresh temp directory."""
    mgr = SparkBotManager()
...
class TestLoadAndSave:
    def test_load_returns_none_when_no_config(self, manager: SparkBotManager):
        assert manager.load_bot_config("nonexistent") is None

    def test_save_then_load_roundtrip(self, manager: SparkBotManager):
        cfg = BotConfig(
            name="My Bot",
            description="d",
            persona="p",
    ...
    def test_load_corrupt_yaml_returns_none(self, manager: SparkBotManager):
...
class TestAtomicWrite:
    def test_save_uses_temp_file_and_replace(self, manager: SparkBotManager, monkeypatch):
    ...
    def test_save_does_not_corrupt_existing_on_failed_write(
        self, manager: SparkBotManager, monkeypatch
    ):
...
class TestMergeBotConfig:
    def test_merge_with_no_existing_uses_defaults(self, manager: SparkBotManager):
        merged = manager.merge_bot_config("brand-new", {"name": "X"})
        assert merged.name == "X"
        assert merged.description == ""
        assert merged.channels == {}

    def test_merge_keeps_existing_when_overrides_omit_field(self, manager: SparkBotManager):
    ...
    def test_merge_treats_none_as_not_provided(self, manager: SparkBotManager):
        manager.save_bot_config(
            "bot-2",
    ...
    def test_merge_ignores_unknown_keys(self, manager: SparkBotManager):
        merged = manager.merge_bot_config(
