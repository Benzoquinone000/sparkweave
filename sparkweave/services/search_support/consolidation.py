...
_logger = logging.getLogger("sparkweave.search.consolidation")


def _metadata_items(value: Any) -> list[tuple[str, Any]]:
    """Return stable template-friendly item pairs from provider metadata."""
...
{% if link_items %}
### Extracted Links
{% for name, url in link_items[:10] %}
- [{{ name }}]({{ url }})
{% endfor %}
{% endif %}
{% if image_items %}
### Images Found
{% for alt, src in image_items[:5] %}
- ![{{ alt }}]({{ src }})
{% endfor %}
{% endif %}""",
    # -------------------------------------------------------------------------
...
class AnswerConsolidator:
    ...
    def consolidate(self, response: WebSearchResponse) -> WebSearchResponse:
        ...
        if self.use_llm:
            ...
            _logger.info("LLM consolidation completed (%s chars)", len(response.answer))
        else:
            ...
            _logger.info("Template consolidation completed (%s chars)", len(response.answer))
    ...
    def _build_provider_context(self, response: WebSearchResponse) -> dict[str, Any]:
        ...
        # -----------------------------------------------------------------
            ...
            context["links"] = links if isinstance(links, Mapping) else {}
            context["images"] = images if isinstance(images, Mapping) else {}
            context["link_items"] = _metadata_items(links)
            context["image_items"] = _metadata_items(images)
    ...
    def _consolidate_with_llm(self, response: WebSearchResponse) -> str:
        ...
        async def _call() -> str:
            ...
            return await llm_complete(
                ...
                model=self.llm_config.get("model") or config.model,
                api_key=self.llm_config.get("api_key") or config.api_key,
                base_url=self.llm_config.get("base_url") or config.base_url,
                api_version=self.llm_config.get("api_version") or config.api_version,
                binding=self.llm_config.get("binding") or config.binding,
