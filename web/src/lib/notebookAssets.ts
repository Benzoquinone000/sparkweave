...
export function hasNotebookAssetOutput(message: ChatMessage) {
  ...
  return Boolean(getMessageDisplayContent(message) || quizQuestions?.length || visualizeResult || mathResult || externalVideoResult || externalImageResult);
}
...
export function buildNotebookAsset({
  ...
  return {
    ...
    metadata: {
      ...
      external_image: externalImageResult ?? null,
