...
export function MessageBubble({
  ...
  return (
    ...
    >
      {!isUser ? (
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-tint-lavender text-brand-purple">
          <Bot size={18} />
        </div>
      ) : null}
      <div className={`max-w-[min(760px,100%)] ${isUser ? "order-first" : ""}`}>
        <div className={`rounded-lg border p-4 shadow-sm ${isUser ? "border-transparent bg-tint-yellow" : "border-line bg-white"}`}>
              ...
              <div className="ml-auto flex gap-2">
                ...
                {onSave && canSaveAsset ? (
                  <button
                    ...
                    aria-label="保存当前结果"
                    className="inline-flex min-h-8 items-center gap-1 rounded-md border border-line bg-white px-2 text-xs text-steel transition hover:border-[#c8c4be] hover:text-brand-purple"
                  >
                    <Save size={13} />
                    保存
                  </button>
      ...
      {isUser ? (
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-brand-purple text-white">
          <UserRound size={18} />
        </div>
      ) : null}
    </motion.article>
    ...
    <div className="mb-3 flex flex-wrap gap-2">
      {attachments.map((attachment, index) =>
        attachment.type === "image" ? (
          ...
          <div key={`${attachment.filename}-${index}`} className="flex items-center gap-2 rounded-lg border border-line bg-white px-3 py-2 text-xs text-steel">
