"""Unit tests for channel-secret handling in the SparkBot manager layer.
...
# ---------------------------------------------------------------------------
# SparkBotInstance.to_dict contract
# ---------------------------------------------------------------------------


def _make_instance() -> SparkBotInstance:
    return SparkBotInstance(
        bot_id="b",
        config=BotConfig(
            name="bot",
            channels={
...
async def test_reload_lock_serialises_concurrent_calls(monkeypatch):
    ...
    from sparkweave.services.sparkbot import SparkBotManager
