...
def test_get_latest_turn_returns_most_recent_completed_turn(store: SQLiteSessionStore) -> None:
    session = asyncio.run(store.create_session())
    first = asyncio.run(store.create_turn(session["id"], capability="chat"))
    asyncio.run(store.update_turn_status(first["id"], "completed"))
    second = asyncio.run(store.create_turn(session["id"], capability="deep_solve"))
    asyncio.run(store.update_turn_status(second["id"], "failed", "boom"))

    latest = asyncio.run(store.get_latest_turn(session["id"]))
