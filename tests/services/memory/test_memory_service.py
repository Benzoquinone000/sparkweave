...
from sparkweave.services.memory import MemoryService
...
async def _rewrite_stream(**_kwargs):
    yield "## Preferences\n- Prefer concise answers.\n\n## Context\n- Working on SparkWeave memory."


def test_memory_service_refresh_turn_writes_rewritten_document(monkeypatch, tmp_path) -> None:
    service = _make_service(tmp_path)
    monkeypatch.setattr("sparkweave.services.memory.llm_stream", _rewrite_stream)
...
def test_memory_service_refresh_turn_skips_when_model_returns_no_change(
    ...
    service = _make_service(tmp_path)
    monkeypatch.setattr("sparkweave.services.memory.llm_stream", _no_change_stream)
