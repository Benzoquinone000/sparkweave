...
def test_learning_effect_summary_command_outputs_report(monkeypatch, tmp_path: Path) -> None:
    class FakeLearningEffectService:
        def demo_summary(self, *, course_id: str = "", window: str = "14d"):
            return {
                ...
                "course_id": course_id,
                "window": window,
                "headline": "practice improved",
                "primary_action": {"id": "nba_1", "title": "retry"},
                "proof_points": [{"label": "evidence", "value": "1 item"}],
                "requirement_alignment": [{"requirement": "learning effect", "status": "ready"}],
                "markdown": "# Learning effect summary\n\n- practice improved\n",
            }
    ...
    assert "practice improved" in output.read_text(encoding="utf-8")
