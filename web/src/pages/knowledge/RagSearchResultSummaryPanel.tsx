  ...
  return (
    <div className="rounded-lg border border-line bg-canvas p-3" data-testid="knowledge-rag-test-summary">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm font-semibold text-ink">来源概览</p>
        <Badge tone={resultStatus?.tone ?? "neutral"}>{resultStatus?.label ?? "已找到来源"}</Badge>
      </div>
      <div className="mt-3 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
        <ConfigFact label="查找服务" value={knowledgeProviderLabel(result.provider)} />
        <ConfigFact label="问题侧重" value={String(result.retrieval_profile || "auto")} />
        <ConfigFact label="来源数量" value={String(result.source_count ?? sources.length)} />
        <ConfigFact label="材料长度" value={`${content.length} 字`} />
      </div>
