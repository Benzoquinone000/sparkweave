...
export function QuestionLabPage() {
  ...
  const defaultKbName = kbItems.find((item) => item.is_default)?.name || "";
  const selectedKb = kbName || defaultKbName || "ai_textbook";
  const selectedKbLabel = kbName || defaultKbName || "不限定资料";
  ...
  return (
    <div className="dt-dynamic-page h-full overflow-y-auto">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-3.5 px-3.5 py-3.5 pb-20 lg:px-4 lg:pb-4">
        <QuestionLabHeader
          mode={mode}
          onTopic={() => setMode("topic")}
          onMimic={() => setMode("mimic")}
        />
        ...
        >
          <Metric label="练习状态" value={statusLabel(status)} detail="准备练习题" icon={<FileQuestion size={19} />} />
          <Metric label="已生成" value={questions.length} detail={`完成 ${completed} · 失败 ${failed}`} icon={<CheckCircle2 size={19} />} />
          <Metric label="资料来源" value={selectedKbLabel} detail="用于出题语境" icon={<Activity size={19} />} />
        </motion.section>

        <motion.section
          className="grid min-h-0 gap-3.5 xl:grid-cols-[360px_minmax(0,1fr)]"
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="space-y-4">
            <Panel>
              <PanelHeader
                title="先设置练习"
                description="填写主题和题型后生成；想练真题风格，就切到试卷仿题。"
                action={
                  running ? (
              ...
              <div className="mt-4 space-y-3">
                ...
                <AnimatePresence mode="wait">
                  {mode === "topic" ? (
                    ...
                    >
                    <FieldShell label="练习主题">
                      <TextInput
                        data-testid="question-topic-input"
                        value={topic}
                        onChange={(event) => setTopic(event.target.value)}
                        placeholder="例如：函数极限与连续、链式法则、操作系统进程调度"
                      />
                    </FieldShell>
                    <FieldShell label="题型">
            ...
            <Panel>
              <PanelHeader title="保存到学习记录" description="把本次练习保存下来，之后可以复盘。" />
              <div className="mt-4 space-y-3">
    ...
    <section className="dt-page-header dt-page-header-accent-pink p-3.5">
      <div className="flex flex-wrap items-start justify-between gap-3.5">
        <div className="min-w-0">
          ...
          <h1 className="mt-1 text-xl font-semibold leading-tight text-ink">生成一组能立刻作答的练习</h1>
          <p className="mt-2 max-w-2xl text-xs leading-5 text-slate-600">
            填一个知识点就能开始，也可以上传试卷生成同风格仿题。生成后直接作答、复盘并保存。
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button tone={mode === "topic" ? "primary" : "secondary"} onClick={onTopic}>
            按知识点
          </Button>
          <Button tone={mode === "mimic" ? "primary" : "secondary"} onClick={onMimic}>
            试卷仿题
          </Button>
        </div>
      </div>
