...
当前代码事实、数据落点和模型上下文注入链路见 [学习画像与长期记忆设计](./learner-profile-memory-design.md)。
...
从 2026-05-02 起，涉及画像的新功能都按下面规则验收，避免画像重新退化成“好看的标签页”：

- **判断要有证据**：薄弱点、偏好、掌握度、下一步建议都必须能回到至少一类学习行为，例如答题、看过、保存、提交反思或用户校准。
- **证据要变行动**：每个画像结论至少服务一个可点击动作，例如进入导学、生成练习、生成图解、找公开视频或复测。
- **行动要能回写**：用户完成动作后，应写回掌握度、资源偏好、错因、学习反思或下一步建议中的至少一项。
- **解释要给用户看**：前端只能展示“为什么这样安排 / 做完会怎样调整”，不能把事件名、JSON、调试日志当作解释。
- **偏好不能被系统自证**：系统“生成过某类资源”只能作为轻证据；只有用户看过、保存过、答过、完成过或明确反馈有帮助，才可强化资源偏好。
...
- 资源生成事件可以进入证据账本用于透明回溯，但不能直接被当作用户偏好；偏好应主要来自 `viewed`、`saved`、`answered`、`completed` 等真实学习行为。
...
画像功能当前已经从“阶段施工”收束为三条稳定主线：

1. **证据账本**：把导学、练习、Notebook、资源使用、对话信号和用户校准写成可追溯事件。
2. **统一画像**：从 Memory、Guide、题目本、Notebook 和证据账本聚合目标、偏好、薄弱点、掌握度和下一步行动。
3. **模型注入**：把画像压缩为短上下文和结构化 hints，进入 LangGraph turn，让 Chat、解题、出题、图解、动画和导学都能使用同一个学习状态。

后续维护时只围绕这三条主线做增强：

- 新增证据来源时，先确认事件能解释“发生了什么、来自哪里、可信度如何”。
- 新增画像字段时，必须能说明来源、置信度、用户是否可以校准。
- 新增前端展示时，首屏只回答“现在该做什么”和“为什么”，详细证据放到二级区域。
- 新增导学或资源推荐策略时，要把结果回写证据账本，形成“建议 -> 行动 -> 反馈 -> 画像更新”的闭环。
