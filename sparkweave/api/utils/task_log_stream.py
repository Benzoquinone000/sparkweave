"""SSE task log streams for knowledge-base jobs."""
...
_PIPELINE_LOGGER_NAMES = [
    ...
    "sparkweave.CustomEmbedding",
    "sparkweave.EmbeddingClient",
    "sparkweave.KnowledgeInit",
]


class _TaskStreamHandler(logging.Handler):
    """Forward selected log records into a task's SSE stream."""
...
class KnowledgeTaskStreamManager:
    ...
    @staticmethod
    def _queue_event(queue: asyncio.Queue[dict[str, Any]], payload: dict[str, Any]) -> None:
        try:
...
def capture_task_logs(task_id: str):
    """Capture selected pipeline logs and forward them to the task stream."""
