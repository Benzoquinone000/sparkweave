...
def test_rag_tool_schema_exposes_agent_retrieval_profile() -> None:
    definition = RAGTool().get_definition()
    parameters = {parameter.name: parameter for parameter in definition.parameters}

    assert "retrieval_profile" in parameters
    assert parameters["retrieval_profile"].required is False
    assert parameters["retrieval_profile"].enum == [
        "auto",
        "fast",
        "concept",
        "exact",
    ...
    assert "Do not guess the underlying index type" in definition.description
...
async def test_rag_tool_forwards_query_and_extra_kwargs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert captured["retrieval_profile"] == "exact"
...
@pytest.mark.asyncio
async def test_external_image_search_tool_wraps_learning_image_service(monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, Any] = {}

    async def fake_recommend_learning_images(**kwargs: Any) -> dict[str, Any]:
        captured.update(kwargs)
        return {
            "success": True,
    ...
    monkeypatch.setattr("sparkweave.services.image_search.recommend_learning_images", fake_recommend_learning_images)

    result = await ExternalImageSearchTool().execute(
        topic="gradient descent",
        learner_hints={"weak_points": ["learning rate"]},
        max_results=2,
    )

    assert result.success is True
    assert result.content == "Found 1 learning image."
    assert captured["topic"] == "gradient descent"
    assert captured["max_results"] == 2
    assert captured["learner_hints"]["weak_points"] == ["learning rate"]
    assert result.metadata["render_type"] == "external_image"
    assert result.metadata["tool_name"] == "external_image_search"
    assert result.sources[0]["type"] == "external_image"
    assert result.sources[0]["image_url"] == "https://example.com/image.png"
