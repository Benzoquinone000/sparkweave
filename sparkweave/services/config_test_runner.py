"""Streamed configuration health checks owned by ``sparkweave``."""
...
def _llm_probe_params() -> dict[str, Any]:
    try:
        return get_agent_params("llm_probe")
    except Exception:
...
def temporary_env(values: dict[str, str]):
    """Temporarily overlay process env values for a single config test."""
...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        probe_params = _llm_probe_params()
    ...
    async def _test_embedding(
        ...
        from sparkweave.services.embedding import EmbeddingClient, EmbeddingConfig
        ...
        vectors = await client.embed(["SparkWeave embedding smoke test"])
    ...
    async def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        if resolved.missing_credentials:
            raise ValueError(
                ...
                "Set profile.api_key or the corresponding provider env var."
            )
        ...
        run.emit("info", "Running search query: SparkWeave configuration health check")
        result = await web_search(
            query="SparkWeave configuration health check",
            provider=provider,
        )
        run.emit(
            "response",
